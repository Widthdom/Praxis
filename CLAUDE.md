...
AI クライアント側でローカル MCP 設定を持てるなら、まず `cdidx` を stdio MCP サーバーとして追加し、通常の調査は shell 実行より構造化ツールを優先する:
...
このリポジトリでは、追跡対象の Claude Code プロジェクト設定もこの方針に合わせ、CLI フォールバックだけでなく `cdidx` MCP のフル機能を使える状態を維持すること。

未導入なら、.NET 不要の自己完結バイナリを curl で入れられる:

```bash
curl -fsSL https://raw.githubusercontent.com/Widthdom/CodeIndex/main/install.sh | bash
```

あるいは .NET 8+ SDK があるなら:
...
```bash
curl -fsSL https://raw.githubusercontent.com/Widthdom/CodeIndex/main/install.sh | bash
# または: dotnet tool update -g cdidx
```

更新/インストールに失敗した場合は、`.cdidx/codeindex.db` があるなら `sqlite3` で直接検索、なければ `rg` にフォールバックし、その旨を明示すること。
...
- 対象ファイルが分かっていてリテラル一致の行・列を取りたいときは `find` を使い、生テキスト検索へ安易に戻らない。
...
- 影響範囲や依存関係の把握には `deps` を使う。指定ファイルに依存している側を見たいときは `--reverse` を使う。シンボル変更の推移的な波及まで見たいときは MCP の `impact_analysis` を使う。
- シンボル変更の推移的な caller 影響を CLI で見たいときは `impact` を使う。
- 潜在的に使われていないコードを探す CLI は `unused`、MCP では `unused_symbols` を使う。
- もっとも参照されているシンボルを洗い出す CLI は `hotspots`、MCP では `symbol_hotspots` を使う。
- 抽出器や graph 対応言語の最新マトリクス確認には `languages` を使う。記憶頼みで決め打ちしない。
- BOM、null byte、U+FFFD、改行混在などエンコーディング系の疑いがあるときは `validate` を使う。
- コメント、文字列、設定、ドキュメント、XAML、SQL、JSON、YAML、TOML、Markdown、HTML、Vue、Svelte などは `search` を優先する。
- FTS ではなく大小文字区別つきの生 substring 一致が欲しいときは `search --exact` を使う。
- 最近の変更に絞って検索したいときは `search --since <ISO-8601>` を使う。
...
- インデックス対象のプレビューだけ見たいときは `index --dry-run` を使う。
- `status --json` の `fold_ready` を確認し、`--exact` の挙動が旧来の ASCII ベースに見えるときは、まず `cdidx backfill-fold` を試し、それでも folded metadata を信用できないときだけ `cdidx . --rebuild` 後の結果を優先する。
- `status --json` の `csharp_symbol_name_ready` も確認し、C# の operator / conversion-operator / indexer の exact-name 解決が古そうなら `symbols --exact-name` や `definition` を信用する前に `cdidx .` を一度走らせる。

`search --json` は `chunk_start_line`、`chunk_end_line`、`snippet_start_line`、`snippet_end_line`、`match_lines`、`highlights` を含む一致中心スニペットを返し、`files --json` はファイルごとの `checksum`、`modified`、`indexed_at` を返し、`inspect --json` はワークスペース鮮度と graph 対応メタデータも返す。未対応言語の graph クエリは「0件」ではなく「未対応」であることが明示される。

graph 対応言語は 18 言語（Python、JS/TS、C#、Go、Rust、Java、Kotlin、Ruby、C/C++、PHP、Swift、Dart、Scala、Elixir、Lua、VB.NET）。F# は `search` を使う。`symbols` は 32 言語をカバーする（上記に加え F#、R、Haskell、Shell、SQL、Terraform、Protobuf、GraphQL、Gradle、Makefile、Dockerfile、Zig、PowerShell、CSS/SCSS など）。

cdidx のバグ報告・機能提案は https://github.com/Widthdom/CodeIndex/issues へ。
...
- `cdidx` 自体のシンボル抽出漏れ、graph 未対応判定の誤り、CLI/MCP の欠落に気づいたときは `suggest_improvement` で上流へフィードバックする

`suggest_improvement` を使う前に既存 Issue を検索して重複を避けること。`CDIDX_GITHUB_TOKEN` が設定されていれば upstream Issue 化される可能性があり、未設定なら `.cdidx/suggestions.json` にローカル保存される。MCP ツールは safety annotation を持つため、クライアントが理解できるなら read-only な `cdidx` クエリは自動承認でよい。
