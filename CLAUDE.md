...
- `deps` for file-level dependency impact analysis; `--reverse` for what depends on a given file.
- `unused` for potentially dead code (graph-supported languages only).
- `hotspots` for most-referenced symbols.
...
- `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files`, `excerpt`, `outline`, `deps`, `unused`, `hotspots`, `status`, `languages`, `validate`, and `index`
- `batch_query` when you need multiple small read-only queries in one round-trip
- `suggest_improvement` when `cdidx` itself appears to miss symbols, misreport graph support, or expose an indexing/CLI/MCP gap that should be fed back upstream (or file at https://github.com/Widthdom/CodeIndex/issues)
...
- 潜在的に使われていないコードを探すなら `unused` を使う（graph 対応言語のみ）。
- もっとも参照されているシンボルを洗い出したいなら `hotspots` を使う。
...
- `search`、`definition`、`references`、`callers`、`callees`、`symbols`、`files`、`excerpt`、`outline`、`deps`、`unused`、`hotspots`、`status`、`languages`、`validate`、`index`
- 小さな read-only クエリをまとめたいときは `batch_query`
- `cdidx` 自体のシンボル抽出漏れ、graph 未対応判定の誤り、CLI/MCP の欠落に気づいたときは `suggest_improvement` で上流へフィードバックする（または https://github.com/Widthdom/CodeIndex/issues に起票する）
