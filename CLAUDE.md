...
cdidx . --dry-run       # preview what would be indexed without mutating the DB
```
...
- Use `inspect` when you already have a candidate symbol and want definition, nearby symbols, references, callers, callees, file metadata, workspace trust signals, and graph-support metadata in one round-trip. Treat `project_root`, `git_head`, `git_is_dirty`, `graph_language`, `graph_supported`, and `graph_support_reason` as part of the answer, not just the hit list.
- Use `definition` when you need the declaration text for a named symbol; add `--body` when the implementation body matters.
- Use `references`, `callers`, and `callees` for symbol-aware graph questions in supported languages. F# call graphs are not supported; use `search` there instead. If a graph command reports unsupported language metadata, treat that as “unsupported,” not “no hits.”
...
- Use `files` to enumerate candidate files before opening them. Prefer `files --since <ISO-8601>` when you want to focus on recently changed code.
- Use `deps` for file-level dependency impact analysis. Add `--reverse` when you need to know what depends on a given file.
- Use `validate` when you suspect encoding trouble, BOMs, null bytes, replacement characters, or mixed line endings.
...
- Add `--exact` to `search` when you need a case-sensitive raw substring match instead of FTS behavior.
- Use `--count` for a quick preflight before pulling full result sets.
...
- `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files`, `excerpt`, `outline`, `deps`, `status`, `languages`, and `index`
...
cdidx . --dry-run       # DB を変更せず、何が対象になるかだけ事前確認
```
...
- 候補シンボルが見えていて、定義・近傍シンボル・参照・caller・callee・ファイル情報・workspace trust signal・graph 対応メタデータを一度に欲しいときは `inspect` を使う。`project_root`、`git_head`、`git_is_dirty`、`graph_language`、`graph_supported`、`graph_support_reason` も単なる付随情報ではなく判断材料として扱う。
- 宣言本文が欲しいときは `definition`、実装本体まで欲しいときは `--body` を付ける。
- graph 系調査では `references`、`callers`、`callees` を使う。F# の call graph は未対応なので `search` を使う。graph コマンドが未対応言語メタデータを返した場合は、「0件」ではなく「未対応」と読むこと。
...
- 候補ファイルの洗い出しには `files` を使う。最近変更されたコードに絞りたいときは `files --since <ISO-8601>` を優先する。
- 影響範囲や依存関係の把握には `deps` を使う。指定ファイルに依存している側を見たいときは `--reverse` を使う。
...
- FTS ではなく大小文字区別つきの生 substring 一致が欲しいときは `search --exact` を使う。
- 全件取得前の概数確認には `--count` を使う。
...
`search`、`definition`、`references`、`callers`、`callees`、`symbols`、`files`、`map`、`inspect`、`outline`、`excerpt`、`languages`、`status`、`deps`、`validate` は cdidx の標準ワークフローに含まれる。`search --json` は `chunk_start_line`、`chunk_end_line`、`snippet_start_line`、`snippet_end_line`、`match_lines`、`highlights` を含む一致中心スニペットを返し、`files --json` はファイルごとの `checksum`、`modified`、`indexed_at` を返し、`inspect --json` はワークスペース鮮度と graph 対応メタデータも返す。未対応言語の graph クエリは「0件」ではなく「未対応」であることが明示される。
...
- `map` でリポジトリ全体を把握
- 1 シンボルの詳細調査は `analyze_symbol` または `inspect` 相当
- `search`、`definition`、`references`、`callers`、`callees`、`symbols`、`files`、`excerpt`、`outline`、`deps`、`status`、`languages`、`index`
- 小さな read-only クエリをまとめたいときは `batch_query`
- `cdidx` 自体のシンボル抽出漏れ、graph 未対応判定の誤り、CLI/MCP の欠落に気づいたときは `suggest_improvement` を使って上流へフィードバックする

MCP が使えるセッションでは、通常調査のためにローカル SQL テンプレートを維持しようとしないこと。SQL フォールバックは MCP と CLI のどちらも使えない、または壊れている場面に限る。

現在のクライアントで MCP が露出していない場合は、存在しないツール名を前提にせず、上の CLI を使うこと。
