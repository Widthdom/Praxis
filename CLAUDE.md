...
- `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files`, `excerpt`, `outline`, `deps`, `impact_analysis`, `unused_symbols`, `symbol_hotspots`, `status`, `languages`, `validate`, `ping`, and `index`
...
- `status --json` の `fold_ready` を確認し、`--exact` の挙動が旧来の ASCII ベースに見えるときは `cdidx . --rebuild` 後の結果を優先する。
...
- `search`、`definition`、`references`、`callers`、`callees`、`symbols`、`files`、`excerpt`、`outline`、`deps`、`impact_analysis`、`unused_symbols`、`symbol_hotspots`、`status`、`languages`、`validate`、`ping`、`index`
