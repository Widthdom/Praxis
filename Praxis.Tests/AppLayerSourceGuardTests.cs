...
public class AppLayerSourceGuardTests
{
    ...
    public void MainPage_MacPlacementScroll_UsesNativeRecognizersForEmptySpaceActions()
    {
        ...
        Assert.Contains("TryGetMacCurrentRootPointerFromAppKit(out rootPoint)", macSource);
        ...
        Assert.Contains("TryGetMacCurrentPlacementRootPointer(out var rootPoint, preferCached: false)", macSource);
        ...
        Assert.Contains("PlacementScroll.Handler?.PlatformView is not UIView placementScrollView", macSource);
        Assert.Contains("var location = recognizer.LocationInView(placementScrollView);", macSource);
        Assert.Contains("var rawViewportPoint = new Point(location.X, location.Y);", macSource);
