...
public partial class MainPage
{
    private async void TriggerStatusFlash(string? message)
    {
        ...
        catch (Exception ex)
        {
            ...
            CrashFileLogger.WriteWarning(nameof(TriggerStatusFlash), $"Status flash animation failed for message length {messageLength} (isError={StatusFlashErrorPolicy.IsErrorStatus(message)}): {safeMessage}");
        }
        finally
        {
    ...
    private Color GetNeutralStatusBackgroundColor()
    {
        ...
        if (pageNativeElement is Microsoft.UI.Xaml.FrameworkElement fe)
        {
            ...
            {
                return Color.FromArgb("#501F242B");
            }

            if (actual == Microsoft.UI.Xaml.ElementTheme.Light)
            {
                return Color.FromArgb("#36FFFFFF");
            }
        }
#endif
        ...
        return theme == AppTheme.Dark
            ? Color.FromArgb("#501F242B")
            : Color.FromArgb("#36FFFFFF");
    }
