...
- `WindowsCommandInputImePolicyTests.cs`: Windows IME ASCII-mode gating (`ShouldForceAsciiImeMode`), conversion-mode normalization, and caret clamp logic.
...
- `WindowsCommandInputImePolicyTests.cs`: Windows IME の ASCII モード適用判定（`ShouldForceAsciiImeMode`）、変換モード正規化、キャレット補正。
