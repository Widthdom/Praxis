...
- [`FileStateSyncNotifierTests.cs`](../Praxis.Tests/FileStateSyncNotifierTests.cs): linked-source の `FileStateSyncNotifier` テスト。sync payload write/read/unexpected publish warning が共有 safe exception-message helper を通り、exception `Message` getter 自体が壊れていても fallback marker を返すことを確認。
