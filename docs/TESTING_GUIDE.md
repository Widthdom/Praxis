...
- [`AppLayerSourceGuardTests.cs`](../Praxis.Tests/AppLayerSourceGuardTests.cs): source-level regression guards for app-layer failure handling that is hard to compile directly into the lightweight test host. Verifies `MainPage` crash-logs XAML/initialization/display-alert failures and only flips its `initialized` gate after successful startup, `MauiClipboardService` honors cancellation tokens for both read/write paths, `SqliteAppRepository` only publishes its shared SQLite connection after schema/bootstrap load succeeds, `App` does not cache fallback error pages and crash-logs shutdown flush failures, and `FileStateSyncNotifier` warning-logs sync-file read retry exhaustion.
...
- [`CommandExecutorTests.cs`](../Praxis.Tests/CommandExecutorTests.cs): linked-source coverage for home-path expansion in empty-tool fallback launches (`~`, `~/...`, `~\\...`), non-home relative path pass-through, quoted-tool normalization, normalized empty/quoted-empty tool detection, and Windows shell launch working-directory override to the user profile.
...
- [`AppLayerSourceGuardTests.cs`](../Praxis.Tests/AppLayerSourceGuardTests.cs): 軽量テストホストへ直接コンパイルしにくい app-layer の失敗処理を、ソースレベルで回帰保護するテスト。`MainPage` が XAML 読込失敗・初期化失敗・エラー表示失敗を crash-log に残しつつ、起動成功後にのみ `initialized` を立てること、`MauiClipboardService` が clipboard 読み書きの両方で `CancellationToken` を尊重すること、`SqliteAppRepository` がスキーマ/bootstrap 読込成功後にのみ共有接続を公開すること、`App` が fallback エラーページをキャッシュせず shutdown flush 失敗も crash-log に残すこと、`FileStateSyncNotifier` が sync ファイル読込リトライ枯渇を warning 記録することを検証する。
...
- [`CommandExecutorTests.cs`](../Praxis.Tests/CommandExecutorTests.cs): linked-source で取り込んだ `CommandExecutor` のホームパス展開テスト。空 `tool` フォールバック時の `~`、`~/...`、`~\\...` を展開し、通常の相対パスはそのまま残すこと、quoted tool path の正規化、正規化後に空になる tool 値の検出、および Windows シェル起動時の作業ディレクトリ上書きを確認。
