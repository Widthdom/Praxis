...
- [`AppLayerSourceGuardTests.cs`](../Praxis.Tests/AppLayerSourceGuardTests.cs): source-level regression guards for app-layer failure handling that is hard to compile directly into the lightweight test host. Verifies `MainPage` crash-logs XAML/initialization/display-alert failures and only flips its `initialized` gate after successful startup, `MauiClipboardService` honors cancellation tokens for both read/write paths, and `SqliteAppRepository` only publishes its shared SQLite connection after schema/bootstrap load succeeds.
