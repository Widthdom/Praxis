...
- `Controls/CommandEntry.cs` / `Platforms/MacCatalyst/Handlers/CommandEntryHandler.cs`
  ...
  - Windows command input uses a dedicated handler to apply native `InputScopeNameValue.AlphanumericHalfWidth` (IME suppression)
...
- Command input suggestion UX:
  ...
  - Windows command input applies native `InputScopeNameValue.AlphanumericHalfWidth` for IME suppression.
...
- `Controls/CommandEntry.cs` / `Platforms/MacCatalyst/Handlers/CommandEntryHandler.cs`
  ...
  - Windows の command 入力は専用ハンドラで `InputScopeNameValue.AlphanumericHalfWidth` を適用し、IME 変換を抑止する
- `Praxis.Core/Logic/*.cs`
...
- command 入力補助:
  ...
  - Windows の command 入力欄はネイティブ `InputScopeNameValue.AlphanumericHalfWidth` を適用して IME 変換を抑止する
