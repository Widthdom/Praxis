...
- [`App.xaml.cs`](../Praxis/App.xaml.cs)
  ...
  - `UnhandledException` handler captures `IsTerminating` state and attempts synchronous `FlushAsync` before process death; also captures non-Exception thrown objects
...
- [`App.xaml.cs`](../Praxis/App.xaml.cs)
  ...
  - `UnhandledException` ハンドラは `IsTerminating` 状態を記録し、プロセス終了前に `FlushAsync` による同期フラッシュを試行する。Exception 以外のスローオブジェクトも記録する
