...
- `Controls/CommandEntry.cs` / `Platforms/MacCatalyst/Handlers/CommandEntryHandler.cs`
  ...
  - macOS command input uses a dedicated control/handler so `Up/Down` suggestion navigation is handled reliably at native `UITextField` level, and enforces ASCII input with filtering via `AsciiInputFilter`
  - Windows command input uses a dedicated handler to apply native `InputScopeNameValue.AlphanumericHalfWidth` on focus and re-apply it while focused (timer + text-change hooks), forces IME open/conversion status back to ASCII via `imm32`, then strips non-ASCII text in `TextChanging` as a safety net
...
- Command input suggestion UX:
  ...
  - macOS command input enforces ASCII-capable input source while focused and blocks/strips non-ASCII input paths (`setMarkedText`, `insertText`, editing-changed safety net), backed by `AsciiInputFilter`.
  - Windows command input applies native `InputScopeNameValue.AlphanumericHalfWidth` on focus and re-applies it while focused (timer + text-change hooks), forces IME open/conversion status back to ASCII via `imm32`, then strips non-ASCII text in `TextChanging` as a safety net.
...
- `Praxis.Tests/AsciiInputFilterTests.cs` covers ASCII-input filtering used by macOS command input IME suppression:
  - marked-text block decision
  - ASCII-only detection
  - non-ASCII stripping behavior (including mixed/full-width input)
- `Praxis.Tests/WindowsCommandInputImePolicyTests.cs` covers Windows command-input IME enforcement policy:
  - focused-only enforcement decision
  - enforcement timer interval sanity
  - conversion-mode normalization (native/full-shape/candidate flags cleared)
  - caret-position clamping after non-ASCII stripping
- `Praxis.Tests/WindowsModalFocusRestorePolicyTests.cs` covers Windows modal/conflict focus-restore policy:
  - editor modal focus recovery when all modal controls are unfocused
  - conflict dialog focus recovery when all conflict action buttons are unfocused
  - non-target cases (already focused / dialog closed / non-Windows)
- `Praxis.Tests/InputClearButtonVisibilityPolicyTests.cs` covers clear-button visibility rule (show only when input is non-empty).
- `Praxis.Tests/ClearButtonGlyphAlignmentPolicyTests.cs` covers clear-button glyph translation policy (`-0.5` on Windows, `0` on non-Windows).
- `Praxis.Tests/NonPublicPropertySetterTests.cs` covers reflection-based writable-property assignment (public/non-public setter success and missing/read-only/type-mismatch failure).
...
- `Praxis.Tests/CommandSuggestionRowColorPolicyTests.cs` covers selected/unselected suggestion-row background color resolution for Light/Dark themes.
...
- `Praxis.Tests/ThemeDarkStateResolverTests.cs` covers effective-dark resolution for explicit (`Light`/`Dark`) and `System` mode (including platform-trait override cases).
...
- `Controls/CommandEntry.cs` / `Platforms/MacCatalyst/Handlers/CommandEntryHandler.cs`
  ...
  - macOS の command 入力は専用コントロール/ハンドラを使い、候補 `↑/↓` をネイティブ `UITextField` レベルで安定処理しつつ、`AsciiInputFilter` で ASCII 入力を強制する
  - Windows の command 入力は専用ハンドラでフォーカス時に `InputScopeNameValue.AlphanumericHalfWidth` を適用し、フォーカス中もタイマー + `TextChanging` フックで再適用する。さらに `imm32` で IME の Open/Conversion 状態を英字入力モードへ戻し、`TextChanging` で非 ASCII 文字を安全網として除去する
- `Praxis.Core/Logic/*.cs`
...
- command 入力補助:
  ...
  - macOS の command 入力欄はフォーカス中に ASCII 入力ソースを維持し、`setMarkedText` / `insertText` / 編集変更時の安全網で非 ASCII 入力を遮断・除去する（`AsciiInputFilter` 使用）
  - Windows の command 入力欄はフォーカス時にネイティブ `InputScopeNameValue.AlphanumericHalfWidth` を適用し、フォーカス中もタイマー + `TextChanging` フックで再適用する。さらに `imm32` で IME の Open/Conversion 状態を英字入力モードへ戻し、`TextChanging` で非 ASCII 文字を安全網として除去する
...
- `Praxis.Tests/SearchFocusGuardPolicyTests.cs` は、macOS の Search フォーカスガード挙動を検証する。
  ...
  - ユーザー起点（例: Search クリアボタン押下）、バックグラウンド状態、または command 再フォーカスポリシー非適用時は Search フォーカスを許可
- `Praxis.Tests/AsciiInputFilterTests.cs` は、macOS command 入力の IME 抑止で使う ASCII フィルタを検証する。
  - marked text の遮断判定
  - ASCII 判定
  - 非 ASCII 除去（混在文字列 / 全角英数を含む）
- `Praxis.Tests/WindowsCommandInputImePolicyTests.cs` は、Windows command 入力の IME 強制ポリシーを検証する。
  - フォーカス中のみ再強制する判定
  - 再強制タイマー間隔の妥当性
  - 変換モード正規化（ネイティブ/全角/候補系フラグの除去）
  - 非 ASCII 除去後のキャレット位置補正
- `Praxis.Tests/WindowsModalFocusRestorePolicyTests.cs` は、Windows のモーダル/競合フォーカス復帰ポリシーを検証する。
  - 編集モーダルで入力欄/操作ボタンすべてのフォーカスが外れた場合の復帰判定
  - 競合ダイアログでアクションボタンすべてのフォーカスが外れた場合の復帰判定
  - 既にフォーカスがある場合、ダイアログが閉じている場合、非 Windows の抑止判定
- `Praxis.Tests/InputClearButtonVisibilityPolicyTests.cs` は、入力欄クリアボタンの表示条件（空文字/null で非表示、文字列ありで表示）を検証する。
- `Praxis.Tests/ClearButtonGlyphAlignmentPolicyTests.cs` は、クリアボタン `x` グリフの座標補正ポリシー（Windows は `-0.5`、非 Windows は `0`）を検証する。
- `Praxis.Tests/ClearButtonRefocusPolicyTests.cs` は、クリア押下後フォーカス復帰の再試行ポリシー（非 Windows は即時1回、Windows は即時+短遅延）を検証する。
- `Praxis.Tests/NonPublicPropertySetterTests.cs` は、リフレクションによる書き込み可能プロパティ設定（public/private setter 成功、未定義/読み取り専用/型不一致失敗）を検証する。
...
- `Praxis.Tests/CommandSuggestionRowColorPolicyTests.cs` は、候補行背景色の選択状態別・ライト/ダーク別の色解決を検証する。
...
- `Praxis.Tests/ThemeDarkStateResolverTests.cs` は、実効ダーク判定（明示 `Light`/`Dark` と `System` 時の trait 優先判定）を検証する。
