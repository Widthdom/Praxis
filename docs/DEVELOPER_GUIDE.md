...
- `MainPage` partial classes
  - `MainPage.xaml.cs`: lifecycle/event wiring and startup orchestration
...
3. Wire app behavior in `Praxis` ViewModel/Services and UI orchestration points (`MainPage.*.cs`, platform handlers) as needed.
4. Verify with:
...
- `MainPage` の partial class 群
  - `MainPage.xaml.cs`: ライフサイクル/イベント接続と起動オーケストレーション
...
3. `Praxis` の ViewModel / Services と UI 調停ポイント（`MainPage.*.cs`、プラットフォームハンドラ）にアプリ動作を接続する
4. 次のコマンドで確認する
   - `dotnet test Praxis.slnx`
