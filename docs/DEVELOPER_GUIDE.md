...
- Windows の command 入力は専用ハンドラでフォーカス時に `InputScopeNameValue.AlphanumericHalfWidth` を適用し、`imm32` で IME の Open/Conversion 状態を英字入力寄りへ「フォーカス取得時に 1 回だけ」補正する（フォーカス中の周期再強制と `TextChanging` 書き換えは行わない）。これは command 欄のテキスト Undo/Redo 粒度を壊さないための妥協。`InputScope` 設定時に `ArgumentException (E_RUNTIME_SETVALUE)` が発生した環境では、一方向フラグを立てて以後の `InputScope` 再設定を停止し、IME フォールバックのみで継続する。
- `Praxis.Core/Logic/*.cs`
