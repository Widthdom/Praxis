...
- Windows custom title bar:
  ...
  - `App.ApplyWindowsThemeChrome` is the single entry point for every theme-dependent Windows chrome attribute. It resolves the effective theme via `App.ResolveWindowsAppThemeIsDark` (MAUI's `Application.Current.UserAppTheme` wins over the OS `RequestedTheme` — important because users toggle the theme inside the app via `Ctrl+Shift+L/D/H` and that override needs to drive the Windows chrome too) and then calls four helpers with the resulting `isDark` boolean: `ApplyWindowsImmersiveDarkMode` (`DwmSetWindowAttribute(DWMWA_USE_IMMERSIVE_DARK_MODE=20, BOOL)` — flips the DWM-painted outer 1 px window border between Light-mode white and Dark-mode near-black; without this the dark-themed window had a visible white outline), `ApplyWindowsRootContentBackground` (WinUI Content panel background brush), `ApplyWindowsClassBackgroundBrush` (`SetClassLongPtrW(GCLP_HBRBACKGROUND, ...)` — the Win32 class brush that the OS uses to paint the new client area before WinUI redraws on resize), and `ApplyWindowsDwmColors` (`DWMWA_BORDER_COLOR=34` + `DWMWA_CAPTION_COLOR=35` — compositor-painted resize-border / caption strip colors). The chrome target color is the page idle background (`Light=#F2F2F2 / Dark=#161616` from `Resources/Styles/Colors.xaml`). Called once at startup from `Window.HandlerChanged`, and again from `MainPage` whenever `Application.Current.RequestedThemeChanged` fires (which MAUI raises for both OS theme changes and explicit `UserAppTheme` assignments). Together these reduce — but do not entirely eliminate — the white strip that the OS paints when the user drags the right or bottom resize handle outward and the WinUI compositor has not yet rasterized the new client area (see Known issues below).
  ...
  - The phantom 32 px row that used to appear above the visible caption-buttons row (especially after resize) was traced to **WinUI 3 `NavigationView`'s template stamping `Margin="32,0,0,0"` onto its internal `ContentGrid` part** whenever `ExtendsContentIntoTitleBar = true`. Walk the `WindowRootView` visual subtree with `VisualTreeHelper`, find the `FrameworkElement` with `x:Name="ContentGrid"`, and force its `Margin` to `Thickness(0)`. Because a `MeasureOverride` re-stamps the 32 px back during resize, also attach a `LayoutUpdated` handler on the same `ContentGrid` (keyed by a `ConditionalWeakTable` so each instance gets exactly one handler) that re-zeroes the `Margin` whenever the template tries to restore it.
  - Things tried that did **not** work and were removed:
    - `AppWindow.TitleBar.ExtendsContentIntoTitleBar = true` alone — left the OS title bar in place under MAUI's WinUI host. Switched to `Microsoft.UI.Xaml.Window.ExtendsContentIntoTitleBar = true` + `OverlappedPresenter.SetBorderAndTitleBar(true, false)`.
    - Transparenting `AppWindowTitleBar.ButtonBackgroundColor` / hover / pressed / foreground (and inactive variants) — redundant once `SetBorderAndTitleBar(true, false)` removed the system caption buttons entirely.
    - Subclassing the HWND with a `WM_NCCALCSIZE` handler that returns 0 to claim the entire window as client area — eliminated the right/bottom resize non-client paint but also stripped the resize border AND the Windows 11 rounded corners, so it was the wrong knob.
    - Reflection-clearing every `*Title*` / `*Caption*` member on `WindowRootView` (`_appTitleBar`, `_appTitleBarHeight=0`, `_useCustomAppTitleBar=false`, `_titleBar`, `WindowTitleBarContent=null`, `AppTitleBarTemplate=null`, `AppTitleBarContainer=null`, `AppTitleBarContentControl=null`, `WindowTitleBarContentControlVisibility=Collapsed`, `WindowTitleBarContentControlMinHeight=0`) — collapsed the WindowRootView's own title-bar slot to zero height, but the phantom row remained because it originated in `NavigationView.ContentGrid`, not in WindowRootView. The reflection null-out is retained as a belt-and-suspenders measure but is not what fixes the phantom row.
    - Setting `NavigationView.IsTitleBarAutoPaddingEnabled = false` — the property's official purpose IS to suppress the 32 px ContentGrid margin under ExtendsContentIntoTitleBar, but on the active Windows App SDK the property reads `false` by default yet the margin still gets applied. Kept as a safety net (we still force it false if a future SDK changes the default) but it is not the load-bearing fix.
    - A programmatic 1-pixel `AppWindow.Resize(width+1)` → `AppWindow.Resize(width)` nudge to trigger layout settle — moved the visible title bar to the correct position, but didn't make the `InputNonClientPointerSource.SetRegionRects` Caption declaration effective. Drag/double-click only worked AFTER the user clicked a caption button (which calls `OverlappedPresenter.Maximize` / `Restore` and forces NC re-evaluation). Switched to forcing OS NC re-evaluation directly via Win32 `SetWindowPos(SWP_FRAMECHANGED)` after activation; that makes the SetRegionRects declaration effective immediately at startup, before the user has touched anything.
  - Diagnostic technique that unblocked the investigation: a one-shot visual-tree dump (since removed) that walked `WindowRootView`'s subtree with `VisualTreeHelper.GetChildrenCount` / `GetChild` and logged each `FrameworkElement`'s `ActualWidth/Height`, `Margin`, attached `Grid.Row` / `Grid.Column`, and `TransformToVisual(WindowRootView)` offset. The dump made the single load-bearing line `ContentGrid size=(984x659) at=(0,32) m=32/0/0/0 r=2c=0` obvious — at that point the fix was a 3-line override. Lesson: when reflection-clearing every plausible MAUI internal yields no progress, pivot from "guess the field name" to "dump the visual tree and read the layout numbers" — the answer is often visible there in one line.
  - The right/bottom resize "white strip" was closed by `App.InstallEraseBkgndSubclass`: a `comctl32!SetWindowSubclass` handler on the HWND that intercepts `WM_ERASEBKGND`, `FillRect`s the client rect with a GDI brush matching the page color, and returns `(LRESULT)1` so the OS skips its default class-brush fill (which on WinUI 3 windows tends to be white regardless of `GCLP_HBRBACKGROUND` overrides — that was why the class brush + DWM color combo only narrowed the strip rather than eliminating it). All other messages pass through to `DefSubclassProc` so rounded corners, resize border, NC hit-testing, and WinUI input handling remain unaffected. The brush is created lazily by `App.UpdateEraseBkgndBrush(isDark)` and recreated on theme toggle (old brush released via `gdi32!DeleteObject` to avoid a GDI leak); the subclass delegate is held in a static field so the marshalled thunk is not collected while the HWND is alive.
...
- Windows カスタムタイトルバー:
  ...
  - `App.ApplyWindowsThemeChrome` がテーマ依存の Windows chrome 属性すべてを集約する単一エントリポイント。`App.ResolveWindowsAppThemeIsDark` で実効テーマを解決し（MAUI の `Application.Current.UserAppTheme` が `Light/Dark` 明示なら採用、`Unspecified` なら OS 追従 `RequestedTheme` — `Ctrl+Shift+L/D/H` でアプリ内テーマを切り替える運用なので、その override を Windows chrome 側にも反映させるため）、`isDark` を 4 つのヘルパーに渡す: `ApplyWindowsImmersiveDarkMode`（`DwmSetWindowAttribute(DWMWA_USE_IMMERSIVE_DARK_MODE=20, BOOL)` — DWM がウィンドウ外周に描く 1 px ボーダー色を Light モードの白系から Dark モードの黒系に切り替える。これを設定しないとダークテーマ時に外周が白く縁取られたまま見える）、`ApplyWindowsRootContentBackground`（WinUI Content パネルの背景ブラシ）、`ApplyWindowsClassBackgroundBrush`（`SetClassLongPtrW(GCLP_HBRBACKGROUND, ...)` — リサイズ中に OS が新しい client 領域を塗るときに使う Win32 クラスブラシ）、`ApplyWindowsDwmColors`（`DWMWA_BORDER_COLOR=34` + `DWMWA_CAPTION_COLOR=35` — DWM コンポジタが描くリサイズ境界／キャプション帯の色）。色は全てページのアイドル背景（`Resources/Styles/Colors.xaml` の `Light=#F2F2F2 / Dark=#161616`）に合わせる。`Window.HandlerChanged` 起動時に 1 回、その後は `MainPage` で `Application.Current.RequestedThemeChanged` をフックして毎回再適用する（MAUI は OS テーマ変化と明示的な `UserAppTheme` 代入の両方で当イベントを発火する）。これにより右端・下端を引いてリサイズアウトした際の白帯は緩和されるが、完全には消えない（後述「Known issues」参照）。
  ...
  - キャプションボタン行のさらに上に出ていた約 32 px の余白行（特にリサイズ後）の原因は、**WinUI 3 `NavigationView` のテンプレートが `ExtendsContentIntoTitleBar = true` 環境下で内部 `ContentGrid` パートに `Margin="32,0,0,0"` を貼ること**だった。`WindowRootView` の子孫を `VisualTreeHelper` で walk し、`x:Name="ContentGrid"` の `FrameworkElement` を見つけて `Margin` を `Thickness(0)` に強制する。さらに `MeasureOverride` がリサイズ中に 32 px を再スタンプし直すので、同じ `ContentGrid` に `LayoutUpdated` ハンドラ（`ConditionalWeakTable` で重複登録を防ぐ）を貼って、テンプレートが Margin を戻したらその場で 0 に上書きし返す。
  - 試したが採用しなかったアプローチ:
    - `AppWindow.TitleBar.ExtendsContentIntoTitleBar = true` 単独 — MAUI の WinUI ホスト下では OS タイトルバーが消えなかった。`Microsoft.UI.Xaml.Window.ExtendsContentIntoTitleBar = true` + `OverlappedPresenter.SetBorderAndTitleBar(true, false)` に切り替えて解決。
    - `AppWindowTitleBar.ButtonBackgroundColor` / hover / pressed / foreground（および Inactive 系）を完全透明にする — `SetBorderAndTitleBar(true, false)` でシステムキャプションボタン自体が消えるため不要。
    - HWND を subclass して `WM_NCCALCSIZE` で 0 を返す（ウィンドウ全体をクライアント領域とみなす） — 右下のリサイズ非クライアント塗りは消えるが、同時にリサイズボーダーと Win11 角丸まで消失するため使えない。
    - `WindowRootView` 側の `*Title*` / `*Caption*` 系メンバすべてのリフレクション null 化（`_appTitleBar` / `_appTitleBarHeight=0` / `_useCustomAppTitleBar=false` / `_titleBar` / `WindowTitleBarContent=null` / `AppTitleBarTemplate=null` / `AppTitleBarContainer=null` / `AppTitleBarContentControl=null` / `WindowTitleBarContentControlVisibility=Collapsed` / `WindowTitleBarContentControlMinHeight=0`） — `WindowRootView` 自身のタイトルバー枠は確かに 0 高さに潰せたが、余白行は依然として残った。32 px の余白は `WindowRootView` ではなく `NavigationView.ContentGrid` 側に由来していたため。リフレクション null 化は保険として残しているが、余白行解決の効いた手ではない。
    - `NavigationView.IsTitleBarAutoPaddingEnabled = false` — 公式の用途はまさにこの 32 px ContentGrid Margin を抑止することだが、現行 Windows App SDK では既定値が `false` にもかかわらず Margin が貼られる。SDK の挙動が将来変わった場合の保険として false を強制し続けているが、これだけでは効かない。
    - 1 ピクセル `AppWindow.Resize(width+1)` → `AppWindow.Resize(width)` のプログラマティック nudge — 視覚的なタイトルバー位置は正しく落ち着くものの、`InputNonClientPointerSource.SetRegionRects` で宣言した Caption 矩形が effective にならなかった。ユーザーがキャプションボタンを押す（`OverlappedPresenter.Maximize` / `Restore` を呼ぶ）と OS の NC 再評価がトリガされて初めてドラッグ／ダブルクリックが効くようになる現象が決定打。最終的に Win32 `SetWindowPos(SWP_FRAMECHANGED)` をアクティブ化後に明示的に呼ぶ方式に切り替えた — これで起動直後（ユーザーが何も触らない状態）から Caption 宣言が effective になる。
  - 調査を前に進めた決定的な手法: `VisualTreeHelper.GetChildrenCount` / `GetChild` で `WindowRootView` 配下を再帰 walk し、各 `FrameworkElement` の `ActualWidth/Height`・`Margin`・添付 `Grid.Row`/`Grid.Column`・`TransformToVisual(WindowRootView)` のオフセットをログに dump する一回限りの diagnostic（実装済みだが現在は削除済み）。dump 結果の `ContentGrid size=(984x659) at=(0,32) m=32/0/0/0 r=2c=0` という 1 行を見つけた瞬間に原因が確定し、その後は 3 行で修正できた。教訓: MAUI の internal フィールドを片っ端から null 化しても進展がないときは「フィールド名を推測」する代わりに「visual tree を dump してレイアウト数値を読む」に切り替える。レイアウトに答えが書いてあることが多い。
  - 右端／下端をリサイズアウトしたときに残っていた「白帯」は `App.InstallEraseBkgndSubclass` で解決した。HWND に `comctl32!SetWindowSubclass` で subclass をアタッチして `WM_ERASEBKGND` を捕捉し、ページ色の GDI ブラシでクライアント領域を `FillRect`、`(LRESULT)1` を返して OS のデフォルト塗り（`GCLP_HBRBACKGROUND` を差し替えても WinUI 3 ウィンドウでは白いまま出ることがある — クラスブラシ + DWM 色だけだと白帯が完全に消えなかった理由がこれ）をスキップさせる。他のメッセージは `DefSubclassProc` に素通しなので、角丸・リサイズ枠・NC ヒットテスト・WinUI 入力は無傷。ブラシは `App.UpdateEraseBkgndBrush(isDark)` が遅延生成し、テーマ変更時に再生成（古いブラシは `gdi32!DeleteObject` で解放、GDI リーク防止）。subclass デリゲートは静的フィールドで保持し、HWND の寿命の間 GC で回収されないようにしている。
