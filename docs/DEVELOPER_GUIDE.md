...
- Placement-area launcher buttons intentionally keep the default arrow cursor on hover and instead attach [`Behaviors/GrabHandCursorBehavior.cs`](../Praxis/Behaviors/GrabHandCursorBehavior.cs), which swaps to a closed-hand "grab" cursor while the primary pointer is pressed (macOS `NSCursor.closedHandCursor`, Windows `InputSystemCursorShape.SizeAll` substitute) so drag-to-reposition reads as a grabbed object instead of a generic clickable target.
  - On Mac Catalyst, placement-area empty-space actions are also wired through a native secondary-button `UILongPressGestureRecognizer` on the `PlacementScroll` native view and a CoreGraphics mouse-button polling fallback. Primary rectangle selection is owned by the polling path so left-button press/drag/release still clears selection and draws the rectangle when MAUI pointer/tap gestures do not hit transparent empty canvas areas; the primary native recognizer is not attached, so launcher-button drag pans remain available. The secondary path opens the create modal with clipboard-backed Arguments, and the polling path also preserves middle-click fallback plus Command/Control-click selection toggles while suppressing the follow-up launch tap.
...
- Mac Catalyst では、配置領域の空きスペース操作を `PlacementScroll` の secondary native `UILongPressGestureRecognizer` と CoreGraphics の mouse-button polling fallback でも処理する。Primary の矩形選択は polling path が所有し、左クリック押下/ドラッグ/解放を selection payload に流すため、空きスペース左クリックで選択解除し、ドラッグで矩形を描画できる。Primary native recognizer は接続せず、ランチャーボタンの drag pan を奪わない。Secondary はクリップボード値を Arguments に入れた作成モーダルを開き、polling path は middle-click fallback、Command/Control+クリックの選択トグルと後続 launch tap 抑止も維持する。
