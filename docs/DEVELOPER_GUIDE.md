...
- [`Services/IErrorLogger.cs`](../Praxis/Services/IErrorLogger.cs) / [`Services/DbErrorLogger.cs`](../Praxis/Services/DbErrorLogger.cs)
  ...
  - `FlushAsync(timeout)` drains the pending queue within a timeout — called during graceful shutdown paths (`ProcessExit`, `IsTerminating`)
...
- [`Services/IErrorLogger.cs`](../Praxis/Services/IErrorLogger.cs) / [`Services/DbErrorLogger.cs`](../Praxis/Services/DbErrorLogger.cs)
  ...
  - `FlushAsync(timeout)` はタイムアウト内で保留キューをドレイン — グレースフルシャットダウンパス（`ProcessExit`、`IsTerminating`）で呼び出される
