...
- Command input suggestion UX:
  ...
  - Clear button hover sets hand cursor on Windows (`ProtectedCursor`) and macOS (`NSCursor.pointingHandCursor`).
...
- command 入力補助:
  ...
  - クリアボタンのホバー時カーソルは Windows で `ProtectedCursor` の Hand、macOS で `NSCursor.pointingHandCursor` を使用する
