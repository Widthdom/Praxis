...
- `MainPage` partial classes
  - `MainPage.xaml.cs`: lifecycle/event wiring and top-level orchestration
  - `MainPage.PointerAndSelection.cs`: drag/drop, selection rectangle, pointer button detection
  - `MainPage.FocusAndContext.cs`: context menu/conflict dialog focus visuals and tab-policy handling
  - `MainPage.InteractionState.cs`: pointer/selection state fields grouped by concern
- `ViewModels/MainViewModel.cs` + partial classes
  - `MainViewModel.cs`: shared state, observable properties, initialization, external sync
  - `MainViewModel.Actions.cs`: create/edit/delete/drag/theme/dock command handlers
  - `MainViewModel.CommandSuggestions.cs`: suggestion list lifecycle, viewport diffing, command-match execution
...
- Windows command input uses a dedicated handler that applies native `InputScopeNameValue.AlphanumericHalfWidth` on focus, then nudges IME open/conversion state toward ASCII once per focus acquisition via `imm32` (no focused-state periodic re-apply and no `TextChanging` rewrite). This is an intentional compromise to preserve native text Undo/Redo granularity in the command box while keeping practical alphanumeric input enforcement. If native `InputScope` assignment throws `ArgumentException (E_RUNTIME_SETVALUE)` on some environments, the handler disables further `InputScope` writes and continues with the IME fallback path.
...
- Dock horizontal scrollbar defaults to hidden and is shown only while the pointer is hovering the Dock region.
...
- Command input suggestion UX:
  ...
  - Windows command input (`MainCommandEntry` + modal `ModalCommandEntry`) applies native `InputScopeNameValue.AlphanumericHalfWidth` on focus and nudges IME open/conversion state toward ASCII once on focus acquisition via `imm32`. Focused-state periodic re-apply and `TextChanging` rewrite are intentionally disabled to avoid degrading text Undo/Redo granularity (compromise).
...
- `MainPage` の partial class 群
  - `MainPage.xaml.cs`: ライフサイクル/イベント接続と上位オーケストレーション
  - `MainPage.PointerAndSelection.cs`: ドラッグ&ドロップ、選択矩形、ポインタボタン判定
  - `MainPage.FocusAndContext.cs`: コンテキストメニュー/競合ダイアログのフォーカス表示と Tab ポリシー
  - `MainPage.InteractionState.cs`: ポインタ/選択の状態フィールドを関心ごとで集約
- `ViewModels/MainViewModel.cs` + partial class 群
  - `MainViewModel.cs`: 共有状態、ObservableProperty、初期化、外部同期
  - `MainViewModel.Actions.cs`: 作成/編集/削除/ドラッグ/テーマ/Dock のコマンド処理
  - `MainViewModel.CommandSuggestions.cs`: 候補表示ライフサイクル、ビューポート差分反映、command 実行解決
...
- Windows の command 入力は専用ハンドラでフォーカス時に `InputScopeNameValue.AlphanumericHalfWidth` を適用し、`imm32` で IME の Open/Conversion 状態を英字入力寄りへ「フォーカス取得時に 1 回だけ」補正する（フォーカス中の周期再強制と `TextChanging` 書き換えは行わない）。これは command 欄のテキスト Undo/Redo 粒度を壊さないための妥協。`InputScope` 設定時に `ArgumentException (E_RUNTIME_SETVALUE)` が発生した環境では、以後の `InputScope` 再設定を停止し、IME フォールバックのみで継続する。
- `Praxis.Core/Logic/*.cs`
...
- Dock の横スクロールバーは通常非表示で、ポインターが Dock 領域をホバーしている間だけ表示する。
...
- Windows の command 入力欄（`MainCommandEntry` / モーダル `ModalCommandEntry`）はフォーカス時にネイティブ `InputScopeNameValue.AlphanumericHalfWidth` を適用し、`imm32` で IME の Open/Conversion 状態を英字入力寄りへ 1 回だけ戻す。フォーカス中のタイマー再強制や `TextChanging` での文字列書き換えは、command 欄の Undo/Redo 粒度を崩す副作用があるため無効化している（妥協）。`InputScope` 設定が `ArgumentException (E_RUNTIME_SETVALUE)` で失敗した環境では、再設定を無効化してフォールバック動作を維持する。
