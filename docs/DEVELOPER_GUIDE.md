...
- [`ViewModels/MainViewModel.cs`](../Praxis/ViewModels/MainViewModel.cs) + partial classes
  ...
  - Handles cross-window sync reload with diff-apply when external button/dock changes are notified, Info-logs receipt/defer/start/finish boundaries for sync diagnostics, and warning-logs reload failures instead of letting the background task fail silently
  - Applies cross-window theme sync by reloading persisted theme on external notifications, Info-logs no-op vs applied paths, and warning-logs theme reload failures
...
- [`Services/FileStateSyncNotifier.cs`](../Praxis/Services/FileStateSyncNotifier.cs)
  ...
  - Tolerates malformed payloads and out-of-range timestamps without faulting the background notifier task, crash-logs subscriber exceptions, and writes crash-file Info/Warning breadcrumbs for written, observed, and malformed payloads
...
- [`MainPage`](../Praxis/MainPage.xaml.cs) の partial class 群
  - [`MainPage.xaml.cs`](../Praxis/MainPage.xaml.cs): ライフサイクル/イベント接続と起動オーケストレーション。XAML 読込失敗を crash-log へ記録し、`viewModel.InitializeAsync()` 成功後にのみ初期化済みへ遷移し、初期化失敗時はフラグを戻して次回 `OnAppearing` で再試行できるようにし、エラー表示ダイアログ自体の失敗も別途 crash-log へ記録する。`OnDisappearing` では `viewModel.NotifyWindowDisappearing()` を呼び、ウィンドウ終了をログに記録する
...
- [`ViewModels/MainViewModel.cs`](../Praxis/ViewModels/MainViewModel.cs) + partial class 群
  ...
  - 外部通知時にボタン/Dock 変更を差分再読込してウィンドウ間同期し、受信・保留・開始・終了の境界を Info ログ化する。再読込失敗はバックグラウンドで握り潰さず warning ログ化する
  - 外部通知受信時に保存済みテーマを再読込して、ウィンドウ間でテーマ同期する。変更なし / 適用済み / 失敗の各分岐もログに残す
...
- [`Services/FileStateSyncNotifier.cs`](../Praxis/Services/FileStateSyncNotifier.cs)
  ...
  - 壊れた payload や範囲外 timestamp でバックグラウンド notifier 経路を落とさず、購読者例外は `CrashFileLogger` に記録する。加えて、payload 書き込み・他インスタンス payload 観測・malformed payload 無視も crash-file breadcrumb として残す
- [`Services/AppStoragePaths.cs`](../Praxis/Services/AppStoragePaths.cs)
