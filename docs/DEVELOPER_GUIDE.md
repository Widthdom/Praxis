...
- Placement-area launcher buttons intentionally keep the default arrow cursor on hover and instead attach [`Behaviors/GrabHandCursorBehavior.cs`](../Praxis/Behaviors/GrabHandCursorBehavior.cs), which swaps to a closed-hand "grab" cursor while the primary pointer is pressed (macOS `NSCursor.closedHandCursor`, Windows `InputSystemCursorShape.SizeAll` substitute) so drag-to-reposition reads as a grabbed object instead of a generic clickable target.
  - On Mac Catalyst, placement-area empty-space actions are wired through primary/secondary native `UILongPressGestureRecognizer` instances on the root native view plus a CoreGraphics mouse-button polling fallback. The native recognizers use simultaneous recognition and `CancelsTouchesInView=false`, ignore launcher-button hit points, and convert coordinates through `PlacementScroll` so empty-space rectangle selection and right-click create keep working when `UIScrollView` stops delivering transparent-area events without stealing launcher-button drag pan or Command-click selection. The secondary path opens the create modal with clipboard-backed Arguments, and the polling path also preserves middle-click fallback plus Command/Control-click selection toggles while suppressing the follow-up launch tap.
...
- Mac Catalyst では、配置領域の空きスペース操作を root native view の primary/secondary native `UILongPressGestureRecognizer` と CoreGraphics の mouse-button polling fallback でも処理する。Native recognizer は simultaneous recognition と `CancelsTouchesInView=false` を使い、ランチャーボタン上の hit point は無視し、座標は `PlacementScroll` 経由で変換する。これにより `UIScrollView` が透明領域イベントを配送しない場合でも、空きスペースの矩形選択と右クリック作成を維持しつつ、ランチャーボタンの drag pan や Command+クリック選択を奪わない。Secondary はクリップボード値を Arguments に入れた作成モーダルを開き、polling path は middle-click fallback、Command/Control+クリックの選択トグルと後続 launch tap 抑止も維持する。
