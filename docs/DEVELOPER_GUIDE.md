...
- [`App.xaml.cs`](../Praxis/App.xaml.cs)
  ...
  - Applies the platform glass backdrop when a window handler attaches and when the effective theme changes: macOS clears the native window/titlebar, installs a native `NSVisualEffectView` popover material behind the MAUI content for the root glass, and keeps text-bearing controls on alpha-backed surfaces rather than parent opacity; Windows applies transparent root chrome, transparent titlebar colors, acrylic blur, and DWM rounded corners through local interop
...
- [`MainPage`](../Praxis/MainPage.xaml.cs) partial classes
  ...
  - [`MainPage.FocusAndContext.cs`](../Praxis/MainPage.FocusAndContext.cs): context/conflict focus visuals, tab-policy handling, and Mac Catalyst native context-menu key capture
  ...
  - [`MainPage.DockAndQuickLook.cs`](../Praxis/MainPage.DockAndQuickLook.cs): dock scrollbar hover visibility, Mac Catalyst scroll-indicator spacing/clipping, and Quick Look popup show/hide/placement
...
## Current UI Notes
- The main launcher page and root layout are explicitly transparent. Mac Catalyst draws the visible launcher base with a native `NSVisualEffectView` popover material inserted behind the MAUI content in [`App.MacBackdrop.cs`](../Praxis/App.MacBackdrop.cs); placement, Dock, and status surfaces remain transparent above it. Editor/conflict modals and the copy-complete notice use solid rounded [`Controls/MaterialFrame.cs`](../Praxis/Controls/MaterialFrame.cs) surfaces so field/button text is not blurred by a modal backdrop, while the Mac native-clear pass skips app-owned `MaterialFrame` hosts to preserve rounded clipping. Command suggestions and Quick Look tooltips intentionally use solid light/dark popup surfaces so text and selected rows do not wash through the launcher backdrop. Windows gets the blurred backdrop from [`App.WindowsBackdrop.cs`](../Praxis/App.WindowsBackdrop.cs), with semi-transparent XAML panels layered above it.
...
- Mac Catalyst also clears layer borders on native window/content wrappers so dark mode does not pick up an extra outer frame around the frosted root.
...
- Placement-area launcher buttons intentionally keep the default arrow cursor on hover and instead attach [`Behaviors/GrabHandCursorBehavior.cs`](../Praxis/Behaviors/GrabHandCursorBehavior.cs), which swaps to a closed-hand "grab" cursor while the primary pointer is pressed (macOS `NSCursor.closedHandCursor`, Windows `InputSystemCursorShape.SizeAll` substitute) so drag-to-reposition reads as a grabbed object instead of a generic clickable target.
  - On Mac Catalyst, placement-area empty-space actions are wired through primary/secondary native `UILongPressGestureRecognizer` instances on the root native view plus a CoreGraphics mouse-button polling fallback. The native recognizers use simultaneous recognition and `CancelsTouchesInView=false`, ignore launcher-button hit points, and convert native recognizer positions through the root view before applying MAUI `PlacementScroll` offsets so empty-space rectangle selection and right-click create keep working when `UIScrollView` stops delivering transparent-area events without stealing launcher-button drag pan or Command-click selection. Native-recognizer rectangle drags stay on native-recognizer coordinates from start to finish. Polling-backed rectangle drags only start while AppKit reports the current pointer inside the Praxis native window, then start from a fresh current AppKit root pointer before falling back to CoreGraphics/UIKit and hover cache. AppKit window coordinates are first scaled into the UIKit root view size because Mac Catalyst can report a native window frame such as `1020x843` while the MAUI root view is `1325x1095`; rectangle selection then applies a small final pointer-tip offset before drag updates use that accepted live AppKit root-coordinate candidate, falling back to the raw screen-coordinate delta and then CoreGraphics absolute-coordinate candidates. During an active drag, the AppKit candidate is allowed to move outside the native window so the rectangle can clamp to the placement viewport instead of stalling at the frame edge, and the cursor is forced back to the arrow so native resize cursors do not take over the selection gesture. Native-recognizer and polling rectangle feedback is drawn as a non-interactive native overlay on the root view from the same native root coordinates, with the fill color applied to both `UIView.BackgroundColor` and `CALayer.BackgroundColor`, so its moving corner and filled interior do not depend on MAUI `SelectionRect` parent or `PlacementScroll` viewport conversion. Short false primary-button reads from CoreGraphics are treated as a release grace period during an active drag so polling does not complete and restart the rectangle mid-drag. On release, Windows and Mac Catalyst both fade rectangle feedback out over `UiTimingPolicy.SelectionRectFadeOutDurationMs`. The secondary path opens the create modal with clipboard-backed Arguments, and the polling path also preserves middle-click fallback while suppressing the follow-up launch tap.
  - Lessons from the Mac placement-selection fixes:
    - Do not mix coordinate sources inside a drag. Native-recognizer drags must stay on recognizer coordinates; polling drags must keep the accepted AppKit/CoreGraphics candidate for the whole gesture.
    - `CGEvent.Location` can be stale or freeze while the primary button is down, so polling movement should prefer live AppKit root coordinates and keep raw screen deltas only as a fallback.
    - AppKit `NSWindow` frame coordinates and UIKit/MAUI root view coordinates can have different sizes on Mac Catalyst. Always scale AppKit window coordinates into the root view before drawing or hit-testing, otherwise the rectangle has proportional drift where the cursor/rectangle gap changes by screen position.
    - The pointer-tip offset is selection-only and must be applied after the AppKit-to-root scale, to both the start point and running point. Right-click create and normal click hit-testing should keep using the unadjusted pointer coordinate.
    - Starting a polling selection should still require the pointer to be inside the Praxis native window, but an active drag must continue outside the window and clamp to the placement viewport. While active, force the arrow cursor so native window resize cursors do not interrupt the selection affordance at frame edges.
    - The native overlay is the visual source of truth for Mac selection feedback. Keep fill and border on the native overlay, apply fill to both `UIView.BackgroundColor` and `CALayer.BackgroundColor`, and keep fade-out on release in the overlay path as well as the Windows/MAUI path.
  - A root native `UIHoverGestureRecognizer` continuously refreshes a placement-specific hover root point in addition to `lastPointerOnRoot`, and the polling path reads the recognizer location on demand only while hover is actively changing. The cached placement point is kept across hover-ended states because UIKit can report `(0,0)` after hover ends just before the mouse-button polling transition arrives. For placement left/right polling, the current AppKit `NSEvent.mouseLocation` plus matched `NSWindow.convertPointFromScreen:` result is tried before cached hover coordinates, so first-click and post-hover empty-space actions use the actual mouse position instead of a stale hover point.
  - `OnDisappearing` detaches those Mac placement recognizers and stops the polling timer, so initialized `OnAppearing` must re-run `ApplyMacVisualTuning()` and `StartMacMiddleButtonPolling()` before returning; otherwise empty-space rectangle selection and right-click create stay disconnected after a hide/show or LaunchServices activation cycle.
- Dock item visuals are intentionally matched to placement-area button visuals.
- Dock keeps a compact minimum height and bottom-biased padding so the borderless layout gives more vertical room back to the placement area while still masking the horizontal scrollbar when it is idle. The Dock button stack keeps an explicit bottom gap above the indicator; on Mac Catalyst the native `UIScrollView` remains clipped at the left/right edges so partially visible buttons end in straight edges, while `ScrollIndicatorInsets` shifts the horizontal indicator down into the reserved gap.
...
- Dock horizontal scrollbar defaults to hidden and is shown only while the pointer is hovering the Dock region and horizontal overflow exists.
  ...
  - Mac Catalyst keeps `UIScrollView.ClipsToBounds = true`; do not disable clipping to make scrollbar spacing easier, because it lets offscreen Dock button rounded corners leak through at the left/right edges.
...
- Selection rectangle is rendered as `SelectionRect` in [`MainPage.xaml`](../Praxis/MainPage.xaml) with gray stroke/fill. `MainPage.PointerAndSelection.cs` converts placement viewport points into the actual `SelectionRect` parent coordinate space before updating `TranslationX` / `TranslationY`, because the overlay is a sibling of `PlacementScroll`, not a child of the scroll view. The shared `UpdateSelectionRect(..., hide: true)` path fades the rectangle out using `UiTimingPolicy.SelectionRectFadeOutDurationMs` before hiding it.
...
- Command input suggestion UX:
  ...
  - On Mac Catalyst, right-click context menus do not use a hidden XAML input field for keyboard capture. `MainPage.FocusAndContext.cs` creates an invisible native `MacContextMenuKeyCaptureView` (`UIView`) when the menu opens, makes it first responder, and handles `Up`/`Down`/`Tab`/`Escape`/`Return` in `PressesBegan` by synchronously dispatching `App.RaiseEditorShortcut(...)`. This keeps arrow-key selection and a single `Return` press aligned with the pseudo-focused `Edit`/`Delete` item, including menus opened from the placement area, Dock, and command suggestions. The view is removed/resigned when the context menu closes.
  ...
  - macOS `Tab`/`Shift+Tab`/`Escape`/`Command+S`/`Enter`/arrow keyboard shortcuts for context menu, editor modal, and conflict dialog are dispatched via `App.RaiseEditorShortcut(...)` from:
    ...
    - `MacContextMenuKeyCaptureView` for right-click context menus after the command input has resigned first responder
...
- [`App.xaml.cs`](../Praxis/App.xaml.cs)
  ...
  - `CreateWindow()` は正常に解決できた `MainPage` だけをキャッシュし、`ResolveRootPage()` のエラー表示ページはキャッシュしない。これにより一時的な起動失敗後でも後続ウィンドウで通常ページ解決を再試行できる
    - window handler 接続時と有効テーマ変更時に platform glass backdrop を適用する。macOS は native window / titlebar を透明化し、root glass には MAUI content 背面の native `NSVisualEffectView` popover material を使い、文字を持つ control は親 opacity ではなく alpha 付き surface に載せる。Windows は透明 root chrome、透明 titlebar 色、acrylic blur、DWM rounded corners をローカル interop で適用する
...
- [`MainPage`](../Praxis/MainPage.xaml.cs) の partial class 群
  ...
  - [`MainPage.FocusAndContext.cs`](../Praxis/MainPage.FocusAndContext.cs): コンテキスト/競合のフォーカス表示、Tab ポリシー、Mac Catalyst の native コンテキストメニュー key capture
  ...
  - [`MainPage.DockAndQuickLook.cs`](../Praxis/MainPage.DockAndQuickLook.cs): Dock スクロールバーのホバー表示制御、Mac Catalyst の scroll indicator 余白/clipping、Quick Look の表示/非表示/配置
...
## 現在の UI 実装メモ
- メインランチャー画面と root layout は明示的に透明化する。Mac Catalyst では [`App.MacBackdrop.cs`](../Praxis/App.MacBackdrop.cs) が MAUI content の背面に native `NSVisualEffectView` popover material を挿入し、それを見える launcher base にする。配置面、Dock 面、Status 面はその上で透明のままにする。Editor / conflict modal と copy complete notice は単色の角丸 [`Controls/MaterialFrame.cs`](../Praxis/Controls/MaterialFrame.cs) surface を使い、modal backdrop の blur で入力欄やボタン文字がぼけないようにする。Mac の native clear 処理は app-owned `MaterialFrame` host を対象外にして角丸 clipping を保持する。Command 候補と Quick Look tooltip は、文字と選択行が背面に流されないよう、意図的に透過しない light/dark popup surface を使う。Windows は [`App.WindowsBackdrop.cs`](../Praxis/App.WindowsBackdrop.cs) の acrylic backdrop を使い、その上に半透明 XAML パネルを重ねる。
...
- Mac Catalyst では native window / content wrapper の layer border も消し、ダークモードで frosted root の外周に余分な枠が残らないようにする。
...
- Mac Catalyst では、配置領域の空きスペース操作を root native view の primary/secondary native `UILongPressGestureRecognizer` と CoreGraphics の mouse-button polling fallback でも処理する。Native recognizer は simultaneous recognition と `CancelsTouchesInView=false` を使い、ランチャーボタン上の hit point は無視し、座標は native recognizer の位置を root view 座標へ変換してから MAUI の `PlacementScroll` offset を適用する。これにより `UIScrollView` が透明領域イベントを配送しない場合でも、空きスペースの矩形選択と右クリック作成を維持しつつ、ランチャーボタンの drag pan や Command+クリック選択を奪わない。Native recognizer の矩形ドラッグは開始から終了まで native recognizer 座標だけで完結する。Polling-backed の矩形ドラッグは AppKit が現在ポインタを Praxis の native window 内と判定した場合だけ開始し、fresh な AppKit root 現在ポインタから開始する。CoreGraphics/UIKit と hover cache は fallback にする。Mac Catalyst では AppKit の native window frame が `1020x843`、MAUI root view が `1325x1095` のように異なるサイズを報告することがあるため、AppKit window 座標はまず UIKit root view サイズへ scale する。その後、矩形選択専用の小さな pointer-tip offset を加え、drag 更新では採用済みの live AppKit root 座標を優先し、raw screen 座標差分と CoreGraphics 絶対座標は fallback にする。Active drag 中は AppKit 候補が native window 外へ出ることを許可し、矩形端は placement viewport に clamp する。さらに native window edge で resize cursor に奪われないよう、選択中は arrow cursor を明示的に再設定する。Native recognizer と polling の表示用矩形は同じ native root 座標から root view 上の非操作 overlay として描き、fill は `UIView.BackgroundColor` と `CALayer.BackgroundColor` の両方へ設定する。これにより MAUI `SelectionRect` の親座標変換や `PlacementScroll` viewport 変換に依存せず、矩形角と塗りを安定させる。Windows / Mac Catalyst とも、release 時の矩形は `UiTimingPolicy.SelectionRectFadeOutDurationMs` で fade out する。Secondary はクリップボード値を Arguments に入れた作成モーダルを開き、polling path は middle-click fallback と後続 launch tap 抑止も維持する。
- Mac 配置矩形選択の修正から得た実装上の注意点:
  - 1 回の drag 中に座標系を混ぜない。Native recognizer で始まった drag は recognizer 座標、polling で始まった drag は採用済みの AppKit/CoreGraphics 候補で最後まで通す。
  - `CGEvent.Location` は primary button 押下中に古くなったり止まったりすることがある。Polling の移動は live AppKit root 座標を優先し、raw screen delta は fallback に限定する。
  - AppKit の `NSWindow` frame 座標と UIKit/MAUI root view 座標は Mac Catalyst では同じ倍率とは限らない。Scale せずに overlay を描くと、左上と右下でカーソルと矩形角の距離が変わる比例ズレになる。
  - Pointer-tip offset は scale 後に、矩形選択の start/running point へだけ適用する。右クリック作成や通常クリックの hit-test には未補正座標を使う。
  - Polling selection の開始は Praxis native window 内に限定する。一方で開始後の drag は window 外でも継続し、placement viewport に clamp する。
  - Native overlay が Mac の選択矩形表示の正とする。塗りは view と layer の両方へ設定し、release fade-out も overlay path で維持する。
- root native `UIHoverGestureRecognizer` で placement 専用の hover root point と `lastPointerOnRoot` を継続更新し、polling path は hover が active に変化している間だけ recognizer の現在位置も直接読み直す。UIKit は hover 終了後、mouse-button polling の押下遷移が届く直前に `(0,0)` を返すことがあるため、hover ended では placement 専用 cache を消さない。配置領域の left/right polling では AppKit の `NSEvent.mouseLocation` と対応する `NSWindow.convertPointFromScreen:` を hover cache より先に使い、初回クリックや hover 後の空きスペース操作が古い hover 座標ではなく実マウス位置で判定されるようにしている。
- Dock ボタンの見た目は、配置領域のボタンと意図的に揃えている。
- Dock は最小高と下寄せの余白をコンパクトにし、横スクロールバーの非表示マスクを維持しつつ、枠線なしレイアウトで浮いた縦方向の余白を配置領域へ戻している。Dock ボタン列はインジケータ上に明示的な下端余白を持つ。Mac Catalyst では native `UIScrollView` の左右端 clipping を維持して、部分表示のボタン端が直線で切れるようにしつつ、`ScrollIndicatorInsets` で水平インジケータを確保済みの余白側へ下げる。
...
- Dock の横スクロールバーは通常非表示で、ポインターが Dock 領域をホバーしており、かつ横オーバーフローがある場合のみ表示する。
  ...
  - Mac Catalyst では `UIScrollView.ClipsToBounds = true` を維持する。スクロールバー余白を作るために clipping を外すと、画面外の Dock ボタンの角丸が左右端から漏れて見えるため避ける。
...
- 矩形選択は [`MainPage.xaml`](../Praxis/MainPage.xaml) の `SelectionRect`（グレーストローク/グレー透過塗り）で描画している。共通の `UpdateSelectionRect(..., hide: true)` は `UiTimingPolicy.SelectionRectFadeOutDurationMs` で fade out してから矩形を非表示にする。
...
- Windows の command 入力欄では、ASCII 強制は `ModalCommandEntry` にだけ適用する。フォーカス時にネイティブ `InputScopeNameValue.AlphanumericHalfWidth` を適用し、`imm32` で IME の Open/Conversion 状態を英字入力寄りへ「即時 + 短遅延の 1 回再試行」で戻す。通常はフォーカス中のタイマー再強制や `TextChanging` での文字列書き換えを無効化して Undo/Redo 粒度への副作用を避けるが、モーダル `ModalCommandEntry` はフォーカス中の英字再強制を有効化して、フォーカス後の手動IME切替を抑止している。`InputScope` 設定が `ArgumentException (E_RUNTIME_SETVALUE)` で失敗した環境では、再設定を無効化してフォールバック動作を維持する。
  ...
  - Mac Catalyst の右クリックコンテキストメニューは、キーボード捕捉に hidden XAML input field を使わない。メニュー表示時に `MainPage.FocusAndContext.cs` が不可視の native `MacContextMenuKeyCaptureView`（`UIView`）を作成して first responder にし、`PressesBegan` で `Up` / `Down` / `Tab` / `Escape` / `Return` を同期的に `App.RaiseEditorShortcut(...)` へ中継する。これにより、配置領域・Dock・Command 候補のいずれから開いたメニューでも、方向キー選択後の `Return` 1 回が pseudo-focus された `Edit` / `Delete` と一致する。コンテキストメニューを閉じると、この view は resign/remove する。
  ...
  - macOS の `Tab`/`Shift+Tab`/`Escape`/`Command+S`/`Enter`/方向キー は、`App.RaiseEditorShortcut(...)` を通して以下からコンテキストメニュー/編集モーダル/競合ダイアログへ中継する。
    ...
    - `MacContextMenuKeyCaptureView`（command 入力欄が first responder を解除した後の右クリックコンテキストメニュー）
