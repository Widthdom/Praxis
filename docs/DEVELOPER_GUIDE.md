...
- [`Services/FileStateSyncNotifier.cs`](../Praxis/Services/FileStateSyncNotifier.cs)
  ...
  - Tolerates malformed payloads and out-of-range timestamps without faulting the background notifier task, crash-logs subscriber exceptions, warning-logs sync-file read retry exhaustion, and writes crash-file Info/Warning breadcrumbs for written, observed, and malformed payloads
...
- [`Services/FileStateSyncNotifier.cs`](../Praxis/Services/FileStateSyncNotifier.cs)
  ...
  - 壊れた payload や範囲外 timestamp でバックグラウンド notifier 経路を落とさず、購読者例外は `CrashFileLogger` に記録する。加えて、payload 書き込み・他インスタンス payload 観測・malformed payload 無視・sync ファイル読込リトライ枯渇も crash-file breadcrumb / warning として残す
- [`Services/AppStoragePaths.cs`](../Praxis/Services/AppStoragePaths.cs)
