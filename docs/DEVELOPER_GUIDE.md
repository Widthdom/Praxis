...
- New-button creation opened by right-clicking the placement area: initialize `Arguments` from the clipboard and select all `ButtonText`.
- New-button creation opened from the create button: place the button at the center of the current visible placement viewport and select all `ButtonText`.
...
- 配置領域を右クリックして新規作成モーダルを開く場合、clipboard から `Arguments` を初期入力し、`ButtonText` を全選択する。
- 新規追加ボタンから新規作成モーダルを開く場合、現在表示中の配置 viewport の中央に button を置き、`ButtonText` を全選択する。
