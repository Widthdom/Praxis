...
- [`Services/IErrorLogger.cs`](../Praxis/Services/IErrorLogger.cs) / [`Services/DbErrorLogger.cs`](../Praxis/Services/DbErrorLogger.cs)
  ...
  - Error-level writes trigger a 30-day retention purge after DB insert; DB write failures and purge failures are warning-breadcrumbed to `crash.log` while the original file-first record remains preserved
  - `FlushAsync(timeout)` waits for both queued entries and already-dequeued in-flight DB writes within the timeout window, and warning-breadcrumbs timeout or unexpected flush failures during graceful shutdown paths (`ProcessExit`, `IsTerminating`)
...
- [`Services/IErrorLogger.cs`](../Praxis/Services/IErrorLogger.cs) / [`Services/DbErrorLogger.cs`](../Praxis/Services/DbErrorLogger.cs)
  ...
  - Error レベル書き込み後に 30 日超過分を削除する。DB 書き込み失敗や purge 失敗は、file-first で記録済みの内容を残したまま `crash.log` に warning breadcrumb を追加する
  - `FlushAsync(timeout)` はタイムアウト内で保留キューだけでなく、すでに dequeue 済みの進行中 DB 書き込みも待機し、timeout や予期しない flush 失敗も `crash.log` に warning breadcrumb を残す — グレースフルシャットダウンパス（`ProcessExit`、`IsTerminating`）で呼び出される
