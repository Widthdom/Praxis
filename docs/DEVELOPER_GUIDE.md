...
- [`App.xaml.cs`](../Praxis/App.xaml.cs)
  ...
  - Applies the platform glass backdrop when a window handler attaches and when the effective theme changes: macOS clears the native window/titlebar so `MaterialFrame` can host native blur views, while Windows applies transparent root chrome, transparent titlebar colors, acrylic blur, and DWM rounded corners through local interop
...
- [`App.xaml.cs`](../Praxis/App.xaml.cs)
  ...
  - window handler 接続時と有効テーマ変更時に platform glass backdrop を適用する。macOS は native window / titlebar を透明化して `MaterialFrame` の native blur view を見せ、Windows は透明 root chrome、透明 titlebar 色、acrylic blur、DWM rounded corners をローカル interop で適用する
