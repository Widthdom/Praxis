...
- Windows カスタムタイトルバー:
  ...
  - 未解決の既知の問題: 右端あるいは下端をドラッグしてリサイズアウトすると、依然として薄い白帯が一瞬出る。Win32 クラスブラシ・DWM 色・WinUI ルート背景でかなり緩和されるが完全には消えず、残る白帯はおそらく WinUI コンポジタの swapchain が新しい client サイズへ再ラスタライズされる前に OS が present するために起きている。本格的に消すには `WM_ERASEBKGND` のフックや、WinUI swapchain パネル側との連動が必要になる見込み。
