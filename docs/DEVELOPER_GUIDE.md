...
- [`ViewModels/MainViewModel.cs`](../Praxis/ViewModels/MainViewModel.cs) + partial classes
  ...
  - [`MainViewModel.CommandSuggestions.cs`](../Praxis/ViewModels/MainViewModel.CommandSuggestions.cs): suggestion list lifecycle, viewport diffing, command-match execution; logs command-not-found events and persists full exceptions for debounced refresh, main-thread dispatch, and repository lookup fallback failures before closing the popup safely
...
- [`Services/CrashFileLogger.cs`](../Praxis/Services/CrashFileLogger.cs)
  ...
  - Full exception chain output: inner exceptions, bounded `AggregateException` traversal with middle/tail sampling and duplicate collapse, `Exception.Data` dictionary, and indented per-exception stack traces
...
- [`Services/IErrorLogger.cs`](../Praxis/Services/IErrorLogger.cs) / [`Services/DbErrorLogger.cs`](../Praxis/Services/DbErrorLogger.cs)
  ...
  - Error entries capture full exception type chains (`InvalidOperationException -> NullReferenceException`), concatenated inner messages, and bounded stack output built from each exception node's `Exception.StackTrace` plus aggregate child index labels instead of `Exception.ToString()`
  ...
  - Error-level writes trigger a 30-day retention purge after DB insert; DB write failures for Error/Warning/Info entries and purge failures all write both the exception body and a warning breadcrumb to `crash.log` while the original file-first record remains preserved
...
- [`Services/CrashFileLogger.cs`](../Praxis/Services/CrashFileLogger.cs)
  ...
  - Info/Warning の message payload が `null` の場合は空行ではなく `(no message payload)` へ正規化して記録する
  - 完全な例外チェーン出力: InnerException、middle/tail sampling と duplicate 集約を含む有界 `AggregateException` 走査、`Exception.Data` 辞書、インデント付きの例外単位スタックトレース
...
- [`Services/IErrorLogger.cs`](../Praxis/Services/IErrorLogger.cs) / [`Services/DbErrorLogger.cs`](../Praxis/Services/DbErrorLogger.cs)
  ...
  - Warning/Info の message payload が `null` でもロガー自体は落ちず、`crash.log` と `ErrorLogEntity.Message` の双方で共通の `(no message payload)` プレースホルダへ正規化する
  - エラーエントリは完全な例外型チェーン（`InvalidOperationException -> NullReferenceException`）、連結された内部メッセージ、`Exception.ToString()` ではなく各例外ノードの `Exception.StackTrace` と aggregate 子 index ラベルから構築した有界スタック出力を記録する
