...
- Windows command input uses a dedicated handler to apply native `InputScopeNameValue.AlphanumericHalfWidth` on focus and re-apply it while focused (timer + text-change hooks), forces IME open/conversion status back to ASCII via `imm32`, then strips non-ASCII text in `TextChanging` as a safety net. If native `InputScope` assignment throws `ArgumentException (E_RUNTIME_SETVALUE)` on some environments, the handler disables further `InputScope` writes and continues with IME + ASCII filtering fallback.
...
- Command input suggestion UX:
  ...
  - Windows command input (`MainCommandEntry` + modal `ModalCommandEntry`) applies native `InputScopeNameValue.AlphanumericHalfWidth` on focus and re-applies it while focused (timer + text-change hooks), forces IME open/conversion status back to ASCII via `imm32`, then strips non-ASCII text in `TextChanging` as a safety net.
...
- Windows の command 入力は専用ハンドラでフォーカス時に `InputScopeNameValue.AlphanumericHalfWidth` を適用し、フォーカス中もタイマー + `TextChanging` フックで再適用する。さらに `imm32` で IME の Open/Conversion 状態を英字入力モードへ戻し、`TextChanging` で非 ASCII 文字を安全網として除去する。一部環境で `InputScope` 設定時に `ArgumentException (E_RUNTIME_SETVALUE)` が発生した場合は、以後の `InputScope` 再設定を停止し、IME + ASCII フィルタのみで継続する。
- `Praxis.Core/Logic/*.cs`
...
- Windows の command 入力欄（`MainCommandEntry` / モーダル `ModalCommandEntry`）はフォーカス時にネイティブ `InputScopeNameValue.AlphanumericHalfWidth` を適用し、フォーカス中もタイマー + `TextChanging` フックで再適用する。さらに `imm32` で IME の Open/Conversion 状態を英字入力モードへ戻し、`TextChanging` で非 ASCII 文字を安全網として除去する。`InputScope` 設定が `ArgumentException (E_RUNTIME_SETVALUE)` で失敗した環境では、再設定を無効化してフォールバック動作を維持する。
