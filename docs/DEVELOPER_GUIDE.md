...
- `Services/IErrorLogger.cs` / `Services/DbErrorLogger.cs`
  - `IErrorLogger` exposes a single `void Log(Exception exception, string context)` method
  - `DbErrorLogger` writes `ErrorLogEntry` records to SQLite via `IAppRepository` and purges entries older than 30 days after each write
...
- `Services/IErrorLogger.cs` / `Services/DbErrorLogger.cs`
  - `IErrorLogger` は `void Log(Exception exception, string context)` 1メソッドのインターフェース
  - `DbErrorLogger` は `IAppRepository` 経由で `ErrorLogEntry` を SQLite に書き込み、書き込み後に 30 日超過分を削除する
