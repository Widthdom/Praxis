...
- Top-bar create button uses a line-art logo (outer hexagon, inscribed circle, inner hexagon, center plus)
...
- While suggestions are visible, switching theme (`Light`/`Dark`/`System`) immediately re-applies suggestion row colors on Windows/macOS.
- Status-bar flash turns red for error-like messages (`Failed` / `error` / `exception` / `not found`, including `Command not found: ...`), otherwise green.
- On macOS, if Enter execution from command box results in `Command not found: ...`, focus is returned to command box so typing can continue immediately.
...
- `Command`/`Search` entries use `Keyboard.Plain` so lowercase input is not auto-capitalized by platform defaults.
- On macOS, command input forces ASCII-capable input source while focused and strips non-ASCII text paths (IME composition/insert/editing safety net).
- On Windows, command input applies native `InputScopeNameValue.AlphanumericHalfWidth` on focus and re-applies it while focused (timer + text-change hooks), and also forces IME open/conversion status back to ASCII mode via `imm32`.
- On Windows, command input also strips non-ASCII text in `TextChanging` as a safety net.
- Command/Search inputs show an in-field circular clear button only while text is non-empty; text rendering keeps right-side space so content does not overlap the clear button.
- Pressing a clear button clears that field and immediately returns focus to the same field (caret visible, ready to type).
- On Windows, clear-button `x` glyph uses a half-pixel translation offset so the `x` intersection aligns with the circle center.
- On Windows, clear-button focus restore uses an immediate+delayed retry to avoid focus loss caused by pointer-tap timing.
- Hovering a visible clear button uses a hand cursor (Windows/macOS).
- On Windows, clear-button hover cursor uses a compatibility path that works across SDK access-level differences.
- Editor modal keyboard behavior:
  ...
  - On Windows, if focus leaves all modal inputs/actions, focus is auto-restored to modal `Command` so `Esc` / `Ctrl+S` keep working.
...
- Conflict dialog keyboard behavior (`Reload latest` / `Overwrite mine` / `Cancel`):
  ...
  - On Windows, if all conflict-action buttons lose focus, focus is restored to the last conflict target (fallback: `Cancel`) so `Esc` still closes immediately.
...
- Theme shortcuts:
  ...
  - On macOS, switching to `System` reverts each window to native appearance (`OverrideUserInterfaceStyle=Unspecified`), and modal/input border-caret colors are recalculated from effective system trait.
...
- ステータス表示とコピー通知
- 上部 Create ボタンは線画ロゴ（外六角形・内接円・内六角形・中央 +）を使用
...
- 候補一覧の表示中にテーマ（ライト/ダーク/システム）を切り替えても、Windows/macOS で候補行の色は即時に再適用されます。
- ステータスバーのフラッシュ色は、エラー系メッセージ（`Failed` / `error` / `exception` / `not found`。`Command not found: ...` を含む）で赤、それ以外で緑になります。
- macOS では、command 欄で Enter 実行して `Command not found: ...` になった場合、続けて入力できるよう command 欄へフォーカスを戻します。
...
- `Command` / `Search` 入力欄は `Keyboard.Plain` を設定し、プラットフォーム既定の自動大文字化を抑止します。
- macOS の Command 入力欄はフォーカス中に ASCII 入力ソースを強制し、非 ASCII 文字（IME 合成/挿入/編集経路）を除去します。
- Windows の Command 入力欄はフォーカス時にネイティブ `InputScopeNameValue.AlphanumericHalfWidth` を適用し、フォーカス中もタイマーと `TextChanging` フックで再適用します。加えて `imm32` で IME の Open/Conversion 状態を英字入力モードへ戻します。
- Windows の Command 入力欄は安全網として `TextChanging` で非 ASCII 文字も除去します。
- Command/Search 入力欄は、文字列が空でないときだけ欄内右端に丸形クリアボタンを表示し、文字がボタンに重ならないよう右側表示領域を確保します。
- クリアボタンを押したときは、対象欄の文字列を消去した後に同じ欄へフォーカスを戻し、キャレット表示のまま入力を継続できます。
- Windows では、クリアボタンの `x` グリフに 0.5px の平行移動補正を適用し、`x` の交点が丸背景の中心と一致するようにしています。
- Windows では、ポインタータップのタイミングでフォーカスが外れるケースを避けるため、クリア押下後のフォーカス復帰を即時＋短遅延の再試行で行います。
- クリアボタン表示中にホバーしたときは指カーソルを使用します（Windows/macOS）。
- Windows では、SDK のアクセス制約差分があっても動作する互換経路でクリアボタンのホバーカーソルを設定します。
- 編集モーダルのキーボード操作:
  ...
  - Windows では、モーダル入力/操作ボタンすべてからフォーカスが外れた場合、`Command` へ自動復帰して `Esc` / `Ctrl+S` を継続して有効にします。
...
- 競合ダイアログ（`Reload latest` / `Overwrite mine` / `Cancel`）のキーボード操作:
  ...
  - Windows では競合アクションボタンすべてからフォーカスが外れた場合、最後に選択していた競合ターゲット（未設定時は `Cancel`）へフォーカスを復帰し、`Esc` で即時キャンセルできます。
...
- テーマ切替ショートカット:
  ...
  - macOS で `System` へ戻す際は、各ウィンドウのネイティブ外観（`OverrideUserInterfaceStyle=Unspecified`）へ復帰し、モーダル/入力欄の枠線・キャレット色を実効 trait で再計算します。
