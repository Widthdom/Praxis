...
- Drag, multi-select, edit, and delete buttons in the placement area
  - Placement-area buttons keep the default arrow cursor on hover and switch to a closed-hand grab cursor only while the primary pointer is pressed, so drag-to-reposition reads as a grabbed object; Dock buttons intentionally keep the pointing-hand cursor on hover to still signal "click to launch"
...
- 配置領域でのドラッグ、複数選択、編集、削除
  - 配置領域のボタンは hover 中は既定の矢印カーソルのままで、主ポインタを押下している間だけ「掴んだ手」の grab カーソルに変わる（ドラッグ移動が「掴んで動かす」操作として読めるように）。Dock のボタンは引き続き hover で pointing-hand になり、「クリックで起動」の示唆を維持する
