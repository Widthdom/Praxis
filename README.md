...
- On Windows, command input applies native `InputScopeNameValue.AlphanumericHalfWidth` on focus and re-applies it while focused (timer + text-change hooks) to return from non-alphanumeric IME modes.
...
- Windows の Command 入力欄はフォーカス時にネイティブ `InputScopeNameValue.AlphanumericHalfWidth` を適用し、フォーカス中もタイマーと `TextChanging` フックで再適用して英字入力モードへ戻します。
