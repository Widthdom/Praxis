...
- Dock keeps a compact glass layout so the borderless placement area can use more vertical space
...
- Glassmorphism shell in both light and dark themes, with transparent app chrome/root surfaces, a native frosted root backdrop, and semi-transparent frosted popup/modal panels on Windows and macOS
...
- Dock はコンパクトな glass layout にし、枠線なしの配置領域へ縦方向の余白を戻す
...
- ライト / ダーク両テーマで glassmorphism を採用し、透明なアプリクローム/root 面と native のすりガラス root backdrop を Windows / macOS の両方で使う。候補/tooltip と編集/conflict モーダルは文字の視認性を優先した単色の角丸 surface に載せる
