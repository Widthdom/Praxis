...
- Keep command inputs (top bar + edit modal `Command`) in alphanumeric mode on macOS/Windows
  - Windows compromise: IME is forced toward ASCII immediately on focus acquisition plus one short delayed retry (not continuous polling), to keep native text Undo/Redo behavior stable.
...
- コマンド入力欄（上部 + 編集モーダル `Command`）を macOS/Windows で英字入力モードに維持
  - Windows の妥協点: IME の英字入力寄せはフォーカス取得直後 + 短い遅延での 1 回再試行のみ（常時ポーリング再強制はしない）。テキストの Undo/Redo 粒度を崩さないため。
- ボタンの高速検索・絞り込み
