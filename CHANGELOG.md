...
### Changed
- The Edit/Delete context menu, modal editor, conflict dialog, and command-suggestion popup now fade in and out instead of toggling instantly, with cancellation tokens so rapid open/close cycles cannot leave an overlay stuck mid-fade; the context menu and command-suggestion popup carry drop shadows so they lift above the workspace, the editor modal's previously near-flat shadow is strengthened to match the conflict dialog, and the editor/context overlays drop their `IsVisible` bindings in favor of code-driven `Opacity`/`IsVisible` toggles routed through a shared `AnimateOverlayFadeAsync` helper that also flips `InputTransparent` during fade-out so clicks cannot hit a half-faded surface
...
- Windows main Command/Search text now uses a black light-mode foreground and applies the same contrast policy directly in the Windows Entry handlers so the WinUI insertion caret remains visible against the light glass input surface
- Modal editor text colors are reapplied on light/dark/system theme switches for both MAUI and native Windows/macOS text controls, the bottom status text is centered, and the modal invert-theme checkbox uses the same pointing-hand hover cursor as other clickable launcher controls
- Windows text selection now defines selected-text foreground and highlight brushes in the native WinUI theme dictionary before controls are created, then refreshes the same resources at runtime so the selection rectangle is tuned around WinUI's selected-text foreground behavior
- Selected placement-area launcher buttons now tint their text purple as well as their background, with separate light/dark contrast colors for normal and inverted button themes on both Windows and Mac
- The placement-area drag selection rectangle now uses a thinner one-pixel stroke so multi-select feedback is less visually heavy
- Windows labels, inputs, editors, and buttons now use `Yu Gothic UI` as the app text font while icon glyph labels continue to use `Segoe MDL2 Assets`
- Windows custom chrome no longer renders the "Praxis" title text in the titlebar
- Windows root glass now combines WinUI `DesktopAcrylicController` with the QuickAccess-style HWND fallback: content is extended into the titlebar, the WinUI presenter hides its standard titlebar fill, a single semi-transparent root tint covers titlebar and body, the QuickAccess-style `WS_CAPTION` resize path is preserved, acrylic is reapplied after focus and resize events without rebuilding the acrylic target, and opaque WinUI/MAUI wrapper backgrounds are cleared after handler attach so the desktop blur reads as frosted glass while leaving the Mac Catalyst native material path unchanged
- Windows custom caption buttons now keep explicit hover/pressed colors while the native title text and native titlebar fill stay hidden, with light-mode hover using the same subtle overlay behavior as dark mode
...
- The full-window XAML root glass fill, dummy root `MaterialFrame`, and large placement/Dock/status material fills were removed so the MAUI page stays genuinely transparent; Mac Catalyst now draws the backmost frosted layer as a single native `NSVisualEffectView` in `NSWindow.contentView`
- The native Mac backmost glass uses popover material at full light/dark alpha, no inner layer corner radius, and repeated startup/layout refreshes so the blur follows the actual rounded window bounds without adding a second RoundRect below the titlebar
- The native Mac backmost glass now sizes from the `NSWindow.contentView` bounds, uses an autoresizing mask, and is reapplied on page resize so the frosted root layer follows window enlargement instead of leaving an old-sized edge behind
- The page/root opacity and transparent backgrounds are reasserted during startup and `OnAppearing` so diagnostic or handler repaint states cannot persist across Mac Catalyst rebuilds
...
- Modal editor input surfaces now use a stronger glass-white backing than the main command/search fields while staying translucent
- The command/search fields now keep the Mac native text controls transparent while a separate thin MAUI glass input surface sits behind them, use a real transparent native background image, clear native private background/wrapper layers recursively after they join the native view tree, hide the normal Mac entry border, preserve the focused underline, and render their Mac placeholders as separate crisp MAUI labels; modal editor entries and Clip Word/Note editors use the same independent input surface approach without tinting the text view itself, modal/Quick Look popup tint is now applied inside the native blur view, small UI text rendering was tightened with native scale settings, and modal copy/action controls now use matching light/dark glass surfaces instead of native button fills
...
### Fixed
- Edit/Delete overlays now close when clicking outside the menu on both Windows and Mac Catalyst, including dark mode, and Edit/Delete, editor, and conflict overlays share a full-window hit target rendered as a `Border` (more reliable than a `Grid` for MAUI iOS gesture pickup) using a near-black `#01000000` tint that matches the existing `PlacementSurface` workaround so it stays visually neutral over both light and dark glass surfaces and does not visually emphasize placement-area button outlines while a modal is open. Editor/conflict hit targets block lower-layer clicks without dismissing their dialogs, with the conflict layer sitting between the conflict panel and any editor modal underneath
...
- Mac Catalyst now reconnects placement-area native recognizers and CoreGraphics mouse-button polling on every initialized `OnAppearing`, restoring empty-space rectangle selection and right-click create after the page has disappeared and reappeared
- Mac Catalyst placement empty-space gestures now attach to the root native view with simultaneous recognizers and placement-scroll coordinate filtering, restoring rectangle selection and right-click create when `UIScrollView` stops delivering empty-space events without blocking launcher-button drag or Command-click selection
- Mac Catalyst now tracks placement-area pointer position with a native hover recognizer, a placement-specific cached hover point that survives hover-ended states, and an AppKit mouse-location fallback that is preferred for left/right polling starts, giving CoreGraphics left/right-button polling a fresh root coordinate even when MAUI pointer events do not fire or clear their root-pointer cache over empty transparent areas
- Mac Catalyst placement rectangle selection now keeps native-recognizer drags on native-recognizer coordinates and polling-fallback drags on locked AppKit/CoreGraphics coordinates, scales AppKit window coordinates into the UIKit root-view size before accepting them, applies a small selection-only pointer-tip offset, keeps the native overlay fill color on both the view and layer, uses only fresh current-pointer coordinates for polling starts before falling back to hover cache, rejects polling starts when AppKit reports the pointer outside the Praxis native window, prefers the accepted live AppKit root-coordinate candidate during polling updates before falling back to raw screen-coordinate delta and locked CoreGraphics absolute coordinates, treats either HID or combined-session CoreGraphics button state as pressed to reduce false release/restart loops, debounces short CoreGraphics primary-button release gaps during drags, and draws both native-recognizer and polling rectangles as a native root-view overlay from the accepted native root coordinates, avoiding stale-cache, cross-window global-button starts, mixed recognizer/polling-coordinate, absolute pointer-coordinate, candidate-switching, UIKit/AppKit scale mismatch, transparent-fill layer drift, intermittent restart, and MAUI `SelectionRect` parent-conversion drift while preserving the polling fallback needed when native primary recognizers do not start on empty space
- Mac Catalyst placement rectangle selection now keeps tracking when the drag leaves the Praxis window and forces the arrow cursor during active selection so the window-resize cursor does not take over at native window edges
