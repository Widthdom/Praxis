...
### Known issues
- Windows: a ~32 px phantom row above the visible caption-buttons row still receives layout space — the reflection null-out keeps `WindowRootView._appTitleBarHeight=0` but `WindowRootViewContainer` (the outer host) appears to apply its own measure-side reserve we have not yet identified. The drag region tracks the real caption-buttons row via `TransformToVisual` so dragging works correctly, but visually the top of the page content sits 32 px lower than it should. Worked-through approaches that did not solve this: nulling every `Title*` / `Caption*` property and field on `WindowRootView` via reflection; subclassing the HWND with a `WM_NCCALCSIZE` handler that returns 0 (removed because it also strips the resize border and Win11 rounded corners — wrong knob)
- Windows: dragging the right or bottom edge outward still flashes a thin white strip during the resize. The Win32 class brush + DWM border color + WinUI root background reduce its frequency / area but do not fully eliminate it. The remaining flash is almost certainly the WinUI compositor's swapchain not being re-rasterized at the new size before the OS presents the frame; a real fix likely requires either intercepting `WM_ERASEBKGND` or coordinating with WinUI's swapchain panel directly, both of which we deferred
...
- Windows: OS タイトルバーを置き換える 30px のカスタムタイトルバー（控えめな minimize / maximize-restore / close キャプションボタン3つ）を追加。WinUI 3 XAML の正式な API である `Microsoft.UI.Xaml.Window.ExtendsContentIntoTitleBar = true` を使う（`AppWindow.TitleBar.ExtendsContentIntoTitleBar` だと MAUI の WinUI ホスト下で OS タイトルバーが残ってしまうため）。これでページの背景色（`Light=#F2F2F2 / Dark=#161616` — `Resources/Styles/Colors.xaml`）がウィンドウ全面を塗り、リサイズ時に右端に出ていた白い「のびしろ」帯も解消される。WinUI が右上隅に描画する OS キャプションボタンは、`AppWindowTitleBar.ButtonBackgroundColor` / `ButtonHoverBackgroundColor` / `ButtonPressedBackgroundColor` / `ButtonForegroundColor`（および Inactive 系）をすべて完全透明に設定して見えなくする。ドラッグ領域は `Microsoft.UI.Input.InputNonClientPointerSource.SetRegionRects(NonClientRegionKind.Caption, ...)` で「タイトルバーのうちキャプションボタン群より左側」を OS に通知し、`WindowTitleBar.SizeChanged` / `WindowTitleBarDragRegion.SizeChanged` / `WindowCaptionButtonsStack.SizeChanged` / `AppWindow.Changed`（`DidPresenterChange` / `DidSizeChange`）で再計算する。これにより、どのウィンドウサイズでもドラッグ移動・ダブルクリックで最大化・Aero Snap が引き続き動く。各ボタンは `OverlappedPresenter.Minimize / Maximize / Restore` と `Window.Close` を呼ぶため、ユーザーが「パフォーマンスオプション」で有効化している「ウィンドウの最小化と最大化のアニメーション」を無効化することなく OS ネイティブの最小化・復元アニメーションが使われる。最大化グリフは `AppWindow.Changed` で `ChromeMaximize` ⇔ `ChromeRestore`（Segoe Fluent Icons）が切り替わる。3 つのボタンは同じ薄いテーマ連動 tint（`Light=#E0E0E0 / Dark=#3A3A3A` ホバー、`Light=#D0D0D0 / Dark=#4A4A4A` 押下）を共有し、close も赤系を使わない。Mac Catalyst は影響を受けず、タイトルバー行は高さ 0 で潰してあるので OS のタイトルバーがそのままクロムを担当する
