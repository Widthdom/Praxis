...
- Mac Catalyst placement rectangle selection now uses only fresh hover/native root coordinates for the polling start point, falls back to the current pointer when the hover cache is stale, prefers primary-recognizer native coordinates while dragging, uses AppKit/CoreGraphics movement deltas only as a fallback, and converts `PlacementScroll` viewport points into the actual `SelectionRect` parent coordinate space before drawing, avoiding stale-cache, absolute pointer-coordinate, and overlay-parent drift while preserving the polling fallback needed when native primary recognizers do not start on empty space
