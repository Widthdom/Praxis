...
- The Mac Catalyst main glass tint was reduced again and now uses native under-window material at full light-mode blur strength, with a slightly reduced dark-mode alpha, so the root RoundRect reads as translucent frosted glass instead of a plain tinted transparent plate
- The native Mac backmost glass keeps blur strength separate from tint strength: the `NSVisualEffectView` stays active while only the layer tint is kept very low to avoid returning to an opaque white plate
...
- The XAML dummy root now acts as a tint-only shell over the native Mac `NSVisualEffectView` root glass, removing the extra rounded stroke that visually boxed the command/search, placement, Dock, and status areas
...
- The command/search fields now keep the Mac native text controls transparent while a separate thin MAUI glass input surface sits behind them, use a real transparent native background image, clear native private background/wrapper layers recursively after they join the native view tree, hide the normal Mac entry border, preserve the focused underline, and use lighter regular-weight placeholder rendering; modal editor entries and Clip Word/Note editors use the same independent input surface approach without tinting the text view itself, copy buttons and modal action buttons clear UIKit button configuration/subview backgrounds, refresh when the app theme changes, and replace native button background images before applying matching light/dark glass controls, small UI text rendering was tightened with native scale settings, and modal/Quick Look popup tint was made more transparent while using stronger native material blur
- Mac Catalyst root glass now uses a stronger native window-background material plus full-strength XAML root `MaterialFrame` blur, keeps the dummy root aligned outside the padded layout, strengthens alpha-backed command/search and modal input surfaces without using parent opacity, and renders modal copy/action buttons as glass `Border` surfaces with transparent command overlays so light/dark mode no longer falls back to native white button fills
