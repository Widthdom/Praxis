...
- The command/search fields now use a lighter glass fill while hiding the normal Mac entry border and preserving the focused underline; modal editor fields and wrapping Clip Word/Note editors get their own native blur/tint layer, copy buttons and modal action buttons use matching pale glass controls, placeholder/button text contrast was tightened, and modal/Quick Look popup tint was made more transparent while keeping native material blur
