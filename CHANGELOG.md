...
- The command/search fields now use transparent XAML backgrounds while the Mac native text controls provide a very thin translucent tint layer over the frosted root surface, use a transparent native background image, clear native private background layers recursively, hide the normal Mac entry border, and preserve the focused underline; modal editor fields and wrapping Clip Word/Note editors get the same native tint/layer-clearing treatment, copy buttons and modal action buttons use matching pale glass controls, small UI text rendering was tightened with system medium native text, and modal/Quick Look popup tint was made more transparent while keeping native material blur
