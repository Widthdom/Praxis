...
- The command/search fields now use transparent XAML backgrounds while the Mac native text controls provide their own blur/tint layer, hiding the normal Mac entry border and preserving the focused underline; modal editor fields and wrapping Clip Word/Note editors get the same native blur/tint treatment, copy buttons and modal action buttons use matching pale glass controls, placeholder/button text contrast and small UI text rendering were tightened with system semibold native text, and modal/Quick Look popup tint was made more transparent while keeping native material blur
