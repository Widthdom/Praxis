...
- The command/search fields now use a lighter glass fill while hiding the normal Mac entry border and preserving the focused underline; modal editor fields, wrapping Clip Word/Note editors, copy buttons, and modal action buttons now use matching pale glass controls, placeholder/button text contrast was tightened, and modal/Quick Look popup tint was made more transparent while keeping native material blur
