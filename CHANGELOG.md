...
- `CrashFileLogger.AppendExceptionChain` and `DbErrorLogger`'s type/message builders now hard-cap `AggregateException` child-edge iterations at the remaining node budget so a repeated-reference aggregate (e.g. `Enumerable.Repeat(sharedEx, 100_000)`) cannot iterate O(N) shared-reference markers. Both loggers also route inner messages through a `SafeMessage` helper that skips `AggregateException.Message` (which would itself expand linearly over every inner exception) in favour of a bounded summary; inner messages are still captured by the normal bounded child traversal
...
- Added a per-aggregate child-edge scan cap (`MaxAggregateChildEdgeScan = 4096`) to both `CrashFileLogger` and `DbErrorLogger` so synchronous work stays O(budget), not O(child count), even for aggregates with millions of repeated references. Trailing children beyond the cap are reported with an explicit `"trailing child(ren) not scanned"` marker so operators know the graph was clipped
...
- Added unbounded-scan regression (`[shared × 99999, uniqueTail]`, tail beyond the 4096 edge-scan cap) that asserts the tail's distinctive GUID marker does NOT appear in any persisted field *and* the explicit `"trailing child(ren) not scanned"` truncation marker IS emitted — direct behavioural guard against a regression that removes the per-aggregate edge cap
