...
- Mac Catalyst placement rectangle selection now uses only fresh hover/native root coordinates for the polling start point, falls back to the current pointer when the hover cache is stale, prefers AppKit/CoreGraphics movement deltas from the accepted start point while dragging before live-root and primary-recognizer fallbacks, and converts `PlacementScroll` viewport points into the actual `SelectionRect` parent coordinate space before drawing, avoiding stale-cache, mixed recognizer/polling-coordinate, absolute pointer-coordinate, and overlay-parent drift while preserving the polling fallback needed when native primary recognizers do not start on empty space
